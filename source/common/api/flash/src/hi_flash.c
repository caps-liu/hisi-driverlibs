...
static HI_S32 Flash_Init(HI_VOID)
{
            ...
            || (HI_FLASH_STAT_INSTALL == g_eDevStat[HI_FLASH_TYPE_NAND_0]))
    {
        ...
        if (fp)
        {
            (HI_VOID)fgets(line, sizeof(line), fp); //skip first line
            ...
            for(i=0; fgets(line, sizeof(line), fp) != 0; i++)
            {
                ...
                if ((fd = open(gPartInfo[i].DevName, O_RDWR)) == -1)
                {
                    ...
                    HI_INFO_FLASH("%s->%d access %s readonly!\n",__func__,__LINE__,gPartInfo[i].DevName);
                    gPartInfo[i].perm = ACCESS_RD;
                } else {
                    HI_INFO_FLASH("%s->%d access %s read and write, i:%d, fd:%x!\n",__func__,__LINE__, gPartInfo[i].DevName, i, fd);
...
HI_S32 HI_Flash_UnusedHandle()
{
    ...
    {
        if(INVALID_FD == gFlashInfo[u8Loop].fd)
        {
            break;
        }
    }
                                     ...
                                     HI_U64 u64Len)
{
    ...
    {
        HI_INFO_FLASH("%s(), %d, not permission to be opened.\n", __FUNCTION__, __LINE__);
        pthread_mutex_unlock(&gFlashMutex);
        return (HI_HANDLE)INVALID_FD;
    }
    ...
    gFlashInfo[hFlash].fd = (HI_S32)(SPAN_PART_HANDLE + hFlash);
    ...
    HI_INFO_FLASH("fd= %d, OpenAddress= %#llx, OpenLen= %#llx \n", gFlashInfo[hFlash].fd, u64Address, u64Len);
    HI_INFO_FLASH("end.\n");
    pthread_mutex_unlock(&gFlashMutex);
                                     ...
                                     HI_CHAR *pPartitionName)
{
       ...
       || strstr((char*)g_au8Bootargs, "hi_sfc:"))
    {
        for(i=0; i<MAX_PARTS; i++)
        {
            if(!strcmp(gPartInfo[i].DevName, pPartitionName) && (gPartInfo[i].FlashType == enFlashType)) //eg: "/dev/mtd* "
            {
                break;
            }
            if(!strcmp(gPartInfo[i].PartName, pPartitionName) && (gPartInfo[i].FlashType == enFlashType))
            {
                break;
            }
        }
        ...
        {
            if(!strcmp(gPartInfo[j].DevName, pPartitionName) && (gPartInfo[j].FlashType == enFlashType)) //eg: "/dev/mtd* "
            {
                break;
            }
            if(!strcmp(gPartInfo[j].PartName, pPartitionName) && (gPartInfo[j].FlashType == enFlashType))
            {
                break;
            }
        }
        ...
        {
            HI_ERR_FLASH("Open %s flash partition failure(fd = %d)!\n", DevName, fd);
            pthread_mutex_unlock(&gFlashMutex);
            if (fd >= 0)
            {
                close (fd);
...
HI_HANDLE HI_Flash_OpenByName(HI_CHAR *pPartitionName)
{
       ...
       || strstr((char*)g_au8Bootargs, "hi_sfc:"))
    {
        ...
        for(j=MAX_PARTS - 1; j>= 0; j--)
        {
            if(!strcmp(gPartInfo[j].DevName, pPartitionName)) //eg: "/dev/mtd* "
            {
                break;
            }
            if(!strcmp(gPartInfo[j].PartName, pPartitionName))
            {
                break;
            }
        }
...
static HI_S32 Compensate_Nand_Address(HI_U64 u64Address, HI_U64 *StartAddr)
{
    ...
    *StartAddr += (unsigned long)((HI_U32)idx << BlockShift);
...
HI_S32 HI_Flash_Erase(HI_HANDLE hFlash, HI_U64 u64Address, HI_U64 u64Len)
{
    ...
    ret = gFlashInfo[hFlash].pFlashopt->raw_erase(gFlashInfo[hFlash].fd, (unsigned long long)StartAddr, u64Len, gFlashInfo[hFlash].OpenAddr, LimitLeng);
    HI_INFO_FLASH("end.\n");
    pthread_mutex_unlock(&gFlashMutex);
...
HI_S32 HI_Flash_Read(HI_HANDLE hFlash, HI_U64 u64Address, HI_U8 *pBuf, HI_U32 u32Len, HI_U32 u32Flags)
{
    ...
    ret = gFlashInfo[hFlash].pFlashopt->raw_read(gFlashInfo[hFlash].fd, (unsigned long long *)&StartAddr, pBuf, u32Len, gFlashInfo[hFlash].OpenAddr, LimitLeng, wtihoob, 1);
...
HI_S32 HI_Flash_Write(HI_HANDLE hFlash, HI_U64 u64Address, HI_U8 *pBuf, HI_U32 u32Len, HI_U32 u32Flags)
{
    ...
    if (erase)
    {
        ...
        ret = gFlashInfo[hFlash].pFlashopt->raw_erase(gFlashInfo[hFlash].fd, (unsigned long long)StartAddr, (unsigned long long)u32EraseLen, gFlashInfo[hFlash].OpenAddr, LimitLeng);
        if (0 >= ret)
        {
    ...
    ret = gFlashInfo[hFlash].pFlashopt->raw_write(gFlashInfo[hFlash].fd, (unsigned long long *)&StartAddr, pBuf, u32Len, gFlashInfo[hFlash].OpenAddr, LimitLeng, wtihoob);
...
HI_S32 HI_Flash_GetInfo(HI_HANDLE hFlash, HI_Flash_InterInfo_S *pFlashInfo)
{
    ...
    if( HI_FLASH_TYPE_EMMC_0 == gFlashInfo[hFlash].FlashType)
    {
        ...
        pFlashInfo->fd = (HI_S32)pstEmmcCB;
