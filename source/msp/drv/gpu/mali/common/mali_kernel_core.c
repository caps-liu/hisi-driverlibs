...
_mali_osk_errcode_t _mali_ukk_open(void **context)
{
	...
	session->pending_jobs_lock = _mali_osk_spinlock_init(_MALI_OSK_LOCKFLAG_ORDERED, _MALI_OSK_LOCK_ORDER_SESSION_PENDING_JOBS);
...
_mali_osk_errcode_t _mali_ukk_close(void **context)
{
...
#ifdef CONFIG_SYNC
	{
		...
		_mali_osk_spinlock_lock(session->pending_jobs_lock);
		...
		_mali_osk_spinlock_unlock(session->pending_jobs_lock);

		_mali_osk_wq_flush();

		_mali_osk_spinlock_term(session->pending_jobs_lock);
