...
PVR_FILE64 PVR_OPEN64(const HI_CHAR *filename, int mode)
{
    ...
    int sysFd;
    HI_U64   fileNodeSize;
    HI_CHAR  fNameOpen[PVR_MAX_FILENAME_LEN + 5];
    PVR_FILE_NAME_FMT_E fileFmt;
...
ssize_t PVR_PREAD64(HI_U8 *pMem, HI_U32 size, PVR_FILE64 file, HI_U64 offset)
{
    ...
    if (PVRFileOffsetMatchNode(offset, pPvrFile->u64StartOfCurFd, pPvrFile->u64FileNodeSize))
    {
        ...
        readed = (HI_U32)PVR_READ((void *)pMem, (size_t)readLen1, sysFd, (off_t)(offset - pPvrFile->u64StartOfCurFd));
    ...
    if (readLen1 != size) /* need another file */
    {
        ...
        readed = (HI_U32)PVR_READ((void*)(pMem+readLen1), (size_t)readLen2, sysFd,(off_t)(offset - pPvrFile->u64StartOfCurFd));
        if (-1 == (HI_S32)readed)
        {
            //perror("PVR_READ...333...");
            return -1;
...
ssize_t PVR_PWRITE64(const void *pMem,  HI_U32 size, PVR_FILE64 file, HI_U64 offset)
{
    ...
    if (PVRFileOffsetMatchNode(offset, pPvrFile->u64StartOfCurFd, pPvrFile->u64FileNodeSize))
    {
        ...
        {
            //printf("bbb--- want to write %d,  acturally %d,  offset:%lld,  u64StartOfCurFd: %lld \n", readLen1, writen, offset, pPvrFile->u64StartOfCurFd);
            return 0;
        }
        else
        {
