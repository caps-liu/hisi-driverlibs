...
#define D32FP(i, s0, s1, s2) { \
    ...
    b0 = a0 + a3;			b3 = MUL32_16T(*cptr++, a0 - a3) << (s0);	\
	b1 = a1 + a2;			b2 = MUL32_16T(*cptr++, a1 - a2) << (s1);	\
	buf[i] = b0 + b1;		buf[15-i] = MUL32_16T(*cptr,   b0 - b1) << (s2); \
	buf[16+i] = b2 + b3;    buf[31-i] = MUL32_16T(*cptr++, b3 - b2) << (s2); \
}
...
void FDCT32(int *buf, int *dest, int offset, int oddBlock, int gb)
{
    ...
    for (i = 4; i > 0; i--) {
		...
		b0 = a0 + a7;	    b7 = MUL32_16T(*cptr++, a0 - a7) << 1;
		b3 = a3 + a4;	    b4 = MUL32_16T(*cptr++, a3 - a4) << 3;
		a0 = b0 + b3;	    a3 = MUL32_16T(*cptr,   b0 - b3) << 1;
		a4 = b4 + b7;		a7 = MUL32_16T(*cptr++, b7 - b4) << 1;

		a1 = buf[1];	    a6 = buf[6];	    a2 = buf[2];	    a5 = buf[5];
		b1 = a1 + a6;	    b6 = MUL32_16T(*cptr++, a1 - a6) << 1;
		b2 = a2 + a5;	    b5 = MUL32_16T(*cptr++, a2 - a5) << 1;
		a1 = b1 + b2;		a2 = MUL32_16T(*cptr,   b1 - b2) << 2;
		a5 = b5 + b6;	    a6 = MUL32_16T(*cptr++, b6 - b5) << 2;

		b0 = a0 + a1;	    b1 = MUL32_16T(COS4_0, a0 - a1) << 1;
		b2 = a2 + a3;	    b3 = MUL32_16T(COS4_0, a3 - a2) << 1;
		buf[0] = b0;	    buf[1] = b1;
		buf[2] = b2 + b3;	buf[3] = b3;

		b4 = a4 + a5;	    b5 = MUL32_16T(COS4_0, a4 - a5) << 1;
		b6 = a6 + a7;	    b7 = MUL32_16T(COS4_0, a7 - a6) << 1;
		b6 += b7;
