...
int MY_LOG10(int x)
{
	...
	/* Flog10(f)=Flog10(f/0x10000)+ Flog10(0x10000<<16)= (Flog10(f)+0x4d104), f is frac 16.16 format */
	if(x<=0)
	   return 0;
	else     	
	   return (10*(Flog10((int)x)+0x4d104))>>16;
}
//#define MY_LOG10(x)  MUL32_16B((Flog10(((int)x)<<16)),10)
#else
#define MY_LOG10(x)  (10*log10(x))
#endif

#if 0
static short *setup_window(int len)
{
    int i;
    short *win_tbl = (short *) malloc(len * sizeof(short));
    ...
    for (i = 0; i < len; i++)
    {
        win_tbl[i] = (0.5 * (1 - cos(2 * M_PI * ((double)i / len)))) * 0x7fff;
    }
...
//static int	BitsNeeded(int PowerOfTwo);
...
static int xscale1[] = {0,	1,	2,	3,   4,   5,   6, 7, 8, 11, 15, 20, 27,
...
int spectrum_do(short *PcmBuf, short channels, short *EnergyOut, int i_band)
{
    ...
    for (b = 0; b < PEAK_COUNT; b++)
    {
        ...
        maxval = 0;
        maxidx = -1;
        for (i = band_start; i < band_stop; i++)
        {
            if (maxval < FftOut[i])
            {
                for (k = 0; k < b; k++)
        ...
        peaks[b].peak  = maxval;
        peaks[b].index = maxidx;
    }
#endif


    //start band
    for (i = 0; i < band_start; i++)
    {
        FftOut [i] = 0;
    }
...
#if 0
    printf("waveform demo EnergyOut[%d]=%d\n", peaks[0].index, EnergyOut[peaks[0].index]);
    printf("waveform demo EnergyOut[%d]=%d\n", peaks[1].index, EnergyOut[peaks[1].index]);
    printf("waveform demo EnergyOut[%d]=%d\n", peaks[2].index, EnergyOut[peaks[2].index]);
    printf("waveform demo EnergyOut[%d]=%d\n", peaks[3].index, EnergyOut[peaks[3].index]);
    printf("waveform demo EnergyOut[%d]=%d\n", peaks[4].index, EnergyOut[peaks[4].index]);
#endif
    return 0;
}
