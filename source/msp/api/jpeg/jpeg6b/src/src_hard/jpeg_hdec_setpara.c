...
static HI_S32 JPEG_HDEC_SetSof(const struct jpeg_decompress_struct *cinfo)
{
		...
		{   /**
			 ...
			 ** CNcomment: 不往DDR写YUV数据并且不缩放 CNend\n
			 **/
	    ...
	    #endif
		{
			...
			JPEG_HDEC_WriteReg(pJpegHandle->pJpegRegVirAddr,\
				                  JPGD_REG_SCALE,              \
							      (HI_S32)(((HI_U32)s32DefaultScale & 0xfffffffc) | pJpegHandle->u32ScalRation));
			#endif
		}
...
static HI_VOID JPEG_HDEC_SetDecARGBPara(const struct jpeg_decompress_struct *cinfo)
{
		...
		**/
		...
		#if 0
		s32CropEndX    = (HI_S32)(pJpegHandle->stJpegSofInfo.u32DecW - 1);
		s32CropEndY    = (HI_S32)(pJpegHandle->stJpegSofInfo.u32DecH - 1);
		#endif
...
#if 0//def CONFIG_JPEG_TEST_CHIP_PRESS
/*****************************************************************************
