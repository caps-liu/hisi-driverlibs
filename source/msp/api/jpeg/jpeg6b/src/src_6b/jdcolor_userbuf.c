			...
			for (col = 0; col < (HI_S32)num_cols; col++)
			{
				 if(  (col >= pJpegHandle->stOutDesc.stCropRect.x)
					&&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				  {
						...
						retYBuf[s32RowYNum+3*u32Count+2] =	range_limit[y + Crrtab[cr]];
						retYBuf[s32RowYNum+3*u32Count+1] =	range_limit[y +
												   ((int) RIGHT_SHIFT(Cbgtab[cb] + Crgtab[cr],
													SCALEBITS))];
						retYBuf[s32RowYNum+3*u32Count]	 =	range_limit[y + Cbbtab[cb]];
			...
			for (col = 0; col < (HI_S32)num_cols; col++)
			{
				...
				if(  (col >= pJpegHandle->stOutDesc.stCropRect.x)
					&&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				  {
						...
						retYBuf[s32RowYNum+3*u32Count] =  range_limit[y + Crrtab[cr]];
						retYBuf[s32RowYNum+3*u32Count+1] =	range_limit[y +
												   ((int) RIGHT_SHIFT(Cbgtab[cb] + Crgtab[cr],
													SCALEBITS))];
						retYBuf[s32RowYNum+3*u32Count+2]   =  range_limit[y + Cbbtab[cb]];
			...
			for (col = 0; col < (HI_S32)num_cols; col++)
			{
				 if(  (col >= pJpegHandle->stOutDesc.stCropRect.x)
					 &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				   {
					      ...
					      cb = GETJSAMPLE(inptr1[col]);
					      cr = GETJSAMPLE(inptr2[col]);
							  retYBuf[s32RowYNum+4*u32Count+2] =  range_limit[y + Crrtab[cr]];
						  retYBuf[s32RowYNum+4*u32Count+1] =  range_limit[y +
				                      ((int) RIGHT_SHIFT(Cbgtab[cb] + Crgtab[cr],
				                                         SCALEBITS))];
						  retYBuf[s32RowYNum+4*u32Count]   =  range_limit[y + Cbbtab[cb]];
			...
			for (col = 0; col < (HI_S32)num_cols; col++)
			{
				 ...
				 if(  (col >= pJpegHandle->stOutDesc.stCropRect.x)
					 &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				   {
						  ...
						  retYBuf[s32RowYNum+4*u32Count]   =  range_limit[y + Crrtab[cr]];
						  retYBuf[s32RowYNum+4*u32Count+1] =  range_limit[y +
												  ((int) RIGHT_SHIFT(Cbgtab[cb] + Crgtab[cr],
																	 SCALEBITS))];
						  retYBuf[s32RowYNum+4*u32Count+2]	 =	range_limit[y + Cbbtab[cb]];
						  retYBuf[s32RowYNum+4*u32Count+3] = 0xFF;
		...
		while (--num_rows >= 0)
		{
			...
			for (col = 0; col < (HI_S32)num_cols; col++)
			{
				...
				if(  (col >= pJpegHandle->stOutDesc.stCropRect.x)
					&&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				{
						retYBuf[s32RowYNum+4*u32Count]	 =	inptr0[col];
						retYBuf[s32RowYNum+4*u32Count+1] =	inptr1[col];
						retYBuf[s32RowYNum+4*u32Count+2] =	inptr2[col];
						retYBuf[s32RowYNum+4*u32Count+3] = 0xFF;
						u32Count++;
				  }
	  ...
	  while (--num_rows >= 0)
	  {
			...
			for (col = 0; col < (HI_S32)num_cols; col++)
			{
				 ...
				 if(   (col >= pJpegHandle->stOutDesc.stCropRect.x)
					  &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				  {
						  retYBuf[s32RowYNum+3*u32Count+2] =  inptr[col];
						  retYBuf[s32RowYNum+3*u32Count+1] =  inptr[col];
						  retYBuf[s32RowYNum+3*u32Count]   =  inptr[col];
		  ...
		  while (--num_rows >= 0) 
		  {
				...
				for (col = 0; col < (HI_S32)num_cols; col++)
				{
					...
					if(   (col >= pJpegHandle->stOutDesc.stCropRect.x)
				         &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				    {
						  retYBuf[s32RowYNum+4*u32Count]   =  inptr[col];
						  retYBuf[s32RowYNum+4*u32Count+1] =  inptr[col];
						  retYBuf[s32RowYNum+4*u32Count+2] =  inptr[col];
						  retYBuf[s32RowYNum+4*u32Count+3] = 0xFF;
						  u32Count++;
				    }
		  ...
		  while (--num_rows >= 0) 
		  {
				...
				for (col = 0; col < (HI_S32)num_cols; col++)
				{
					...
					if(   (col >= pJpegHandle->stOutDesc.stCropRect.x)
				         &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				    {
						  retYBuf[s32RowYNum+4*col+1] =  inptr[col];
						  retYBuf[s32RowYNum+4*col+2] =  inptr[col];
						  retYBuf[s32RowYNum+4*col+3] =  inptr[col];
						  retYBuf[s32RowYNum+4*col]	= 0xFF;
						  u32Count++;
				    }  
		...
		while (--num_rows >= 0)
		{
			...
			for (col = 0; col < (HI_S32)num_cols; col++)
			{
				   ...
				   {	
						...
						retYBuf[s32RowYNum+3*u32Count+2] =	(u8YCMK[K]*u8YCMK[C])/u8MaxVal;
						retYBuf[s32RowYNum+3*u32Count+1] =	(u8YCMK[K]*u8YCMK[M])/u8MaxVal; 
						retYBuf[s32RowYNum+3*u32Count]	 =	(u8YCMK[K]*u8YCMK[Y])/u8MaxVal;
...
#if 0
		 JPEG_TRACE("===============================================================\n");
		 JPEG_TRACE("cinfo->output_width       = %d\n",cinfo->output_width);
		 JPEG_TRACE("cinfo->output_height      = %d\n",cinfo->output_height);
	     JPEG_TRACE("cinfo->output_scanline    = %d\n",cinfo->output_scanline);
	     JPEG_TRACE("stOutSurface.u32OutStride = %d\n",pJpegHandle->stOutDesc.stOutSurface.u32OutStride[0]);
	     JPEG_TRACE("stCropRect.x = %d\n",pJpegHandle->stOutDesc.stCropRect.x);
	     JPEG_TRACE("stCropRect.y = %d\n",pJpegHandle->stOutDesc.stCropRect.y);
	     JPEG_TRACE("stCropRect.w = %d\n",pJpegHandle->stOutDesc.stCropRect.w);
	     JPEG_TRACE("stCropRect.h = %d\n",pJpegHandle->stOutDesc.stCropRect.h);
         JPEG_TRACE("===============================================================\n");
#endif
