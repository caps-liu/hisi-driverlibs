...
#define CHECK_AENC_OPEN_FORMAT(rate, ch, width, bInterleaved) \
    do                                                  \
    {                                                   \
        if (rate <  HI_UNF_SAMPLE_RATE_16K || rate > HI_UNF_SAMPLE_RATE_48K)    \
        {                                           \
            HI_ERR_AENC("invalid  Pcm Format: HA Encoder only support 16K~48K samplerate \n");   \
        ...
        if (2 != ch)                  \
        {                                           \
            HI_ERR_AENC("invalid Pcm Format: HA Encoder only support 2 channel\n");   \
            return HI_ERR_AENC_INVALID_PARA;           \
        }                                           \
    } while (0)
...
extern HI_S32 AENC_Open(HI_HANDLE *phAenc, const HI_UNF_AENC_ATTR_S *pstAencAttr);
extern HI_S32 AENC_Pull(HI_HANDLE hAenc);
extern HI_S32 AENC_ReceiveStream (HI_HANDLE hAenc, AENC_STREAM_S *pstStream, HI_U32 u32TimeoutMs);
...
extern HI_S32 AENC_SetEnable(HI_HANDLE hAenc, HI_BOOL bEnable);
extern HI_S32 AENC_AttachInput(HI_HANDLE hAenc, HI_HANDLE hSource);
extern HI_S32 AENC_DetachInput(HI_HANDLE hAenc);
extern HI_S32 AENC_SetAttr(HI_HANDLE hAenc, const HI_UNF_AENC_ATTR_S *pstAencAttr);
extern HI_S32 AENC_GetAttr(HI_HANDLE hAenc, HI_UNF_AENC_ATTR_S *pstAencAttr);
