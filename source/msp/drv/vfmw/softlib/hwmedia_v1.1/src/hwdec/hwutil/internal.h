...
#define av_abort()      do { av_log(NULL, AV_LOG_WARNING, "Abort at %s:%d\n", __FILE__, __LINE__); abort(); } while (0)
...
#define CHECKED_ALLOCZ(avctx,p, size)\
{\
 /*   p= av_mallocz(size);*/\
      p= av_malloc_hw(avctx,size);\
    if(p==NULL && (size)!=0){\
/*      av_log(NULL, AV_LOG_ERROR, "Cannot allocate memory.");*/\
		av_log(NULL, AV_LOG_WARNING, "Cannot allocate memory.");\
        goto fail;\
    }\
	memset(p,0,size);\
}
...
{
    return (float)((x > 0) ? floor(x) : ceil(x));
}
#endif /* HAVE_TRUNCF */
