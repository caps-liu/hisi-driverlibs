...
#include "dsputil.h"
#include "bitstream.h"
//#include "ratecontrol.h"       /* removed by lxw */
#include "parser.h"
#include "mpeg12data.h"
#include "rl.h"
#include "internal.h"   // add by lxw

#define MAX_SLICES 128   // add by lxw
...
//  int ref_count[2][2];        ///< number of entries in ref_poc              (FIXME need per slice)
	...
	int ref_count[2][MAX_SLICES][2];        ///< number of entries in ref_poc              (FIXME need per slice)
	...
	/* 为保证B帧做直接预测的正确性，添加以下语法元素 */
	uint16_t *slice_table_base;
	int slice_table_base_size;
	uint16_t *slice_table;
	int slice_table_size;
...
//    RateControlContext rc_context; ///< contains stuff only accessed in ratecontrol.c /* removed by lxw */
...
#define SLICE_NOEND     -3 ///<no end marker or error found but mb count exceeded
	...
	/*设置MPEG-4 VOL头解析标志，VOL头解析错误时，置位该标志位*/
	int iNotFoundVOLFlag;

	/*xiongfei添加用来标记参考帧方向 20100401*/
	int iMvFlag;
	int iExtFlag; //扩展标志位 x00141957 20100511
	/* 2010/04/08 14:30:00 liuxw+00139685 */
	/* 增加数据成员，以便于某些函数在解析到错误的时候可以直接退出(avoid segment) */
	int iRet;

	/*x00141957 20100726添加 给mpeg2所使用图像显示高宽大小*/
	int iDisplayHeight;
	int iDisplayWeight;

	/* 2010/08/13 11:30:00 liuxw+00139685 */
	/* 由于局部函数变量无法在复位时将其清0，导致复位后再调用时一些内存没有分配，导致死机 */
//	int done;
