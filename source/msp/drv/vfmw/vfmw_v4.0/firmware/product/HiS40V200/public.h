...
#include "stdarg.h" // linux下定义va_list的头函数
#endif
...
#define  VFMW_VERSION_NUM       2013062501   // VFMW的版本号


// limit data  
#define  MAX_QUEUE_IMAGE_NUM    64  /* 此数值不可以随意修改，如果设为非2整数次幂，或者小于64，都可能出问题 */
...
#define  DEFAULT_PRINT_ENABLE   0x0//0x18 //(1<< PRN_DNR)//(1<<PRN_VDMREG)|(1<<PRN_DNMSG) //0x60 //0x0  //0x3B // 默认 'PRN_CTRL' & 'PRN_FATAL' 打开，其余关闭
...
extern  SEM               g_SEM_VdmMutex;    // VDM在视频解码和JPEG解码之间的互斥
extern  SEM               g_SEM_VdmFinish;   // 非H.264协议VDM解码完成信号量

extern  SEM               g_SEM_StateMutex;  // "状态刷新"和"状态获取"之间的互斥
extern  SEM               g_SEM_CfgMutex;    // "配置信息查阅"和"配置信息更新"之间的互斥
...
do{                                                         \
    ...
    {                                                       \
        IMAGE*  para[1];                                    \
        para[0] = pImage;                              \
        g_event_report( chan_id, EVNT_CAPTURE_BTL_OVER, (IMAGE*)para );  \
    }                                                       \
}while(0)
...
#if 0
#ifdef VFMW_DPRINT_SUPPORT
#define dprint dprint_vfmw
#else
#define dprint(type, fmt...)
#endif
#endif
/***********************************************************************
      functions 
