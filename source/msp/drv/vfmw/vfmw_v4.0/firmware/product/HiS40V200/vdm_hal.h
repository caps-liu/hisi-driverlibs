...
#define    MAX_IMG_WIDTH_IN_MB         	352//512 // 120//5632
#define    MAX_IMG_HALF_HEIGHT_IN_MB  	132// 256 // 34//4224
...
do {                    \
    dat = *((volatile SINT32*)((SINT32)g_HwMem[0].pBpdRegVirAddr + reg)); \
} while(0)

#define WR_BPD_VREG( reg, dat )               \
do {                    \
    *((volatile SINT32*)((SINT32)g_HwMem[0].pBpdRegVirAddr + reg)) = dat; \
} while(0)
...
typedef struct
{
    ...
    SINT32    TotalMemUsed;
    // frame buffer
    SINT32    FrameAddr[MAX_FRAME_NUM];
	#ifdef VFMW_MODULE_LOWDLY_SUPPORT
	//add by l00225186 fordsp 2013-04-25
	SINT32    LineNumAddr[MAX_FRAME_NUM];
	#endif
    SINT32    ChromOffset;
    SINT32    ValidFrameNum;
    // btl buffer
    SINT32    BtlAddr[MAX_FRAME_NUM];
...
typedef struct hiVDMHAL_FUN_PTR_S
{
    ...
    SINT32 (*pfun_VDMHAL_OpenHAL)(SINT32 MemBaseAddr, SINT32 Size, SINT32 VdhId);
    VOID (*pfun_VDMHAL_CloseHAL)(SINT32 VdhId);
	SINT32 (*pfun_VDMHAL_ArrangeMem)( SINT32 MemAddr, SINT32 MemSize, SINT32 Width, SINT32 Height, SINT32 PmvNum, SINT32 FrameNum, SINT32 UserDec, SINT32 ChanID, VDMHAL_MEM_ARRANGE_S *pVdmMemArrange );
    ...
    SINT32 (*pfun_VDMHAL_CheckInt)( SINT32 VdhId );	
    ...
    SINT32 (*pfun_VDMHAL_MakeDecReport)( VID_STD_E VidStd, VDMHAL_DEC_REPORT_S *pDecReport, SINT32 VdhId );
    SINT32 (*pfun_VDMHAL_BackupInfo)(UINT32 *pMb0QpInCurrPic, SINT32 VdhId);
    VOID (*pfun_VDMHAL_Write1DYuv)( FILE *fpYuv, UINT8 *Yaddress, UINT32 Width, UINT32 Height, UINT32 chroma_idc);
}VDMHAL_FUN_PTR_S;
...
/*the same reason as VDMHAL_GetHalMemSize*/
#define VDMHAL_OpenHAL(MemBaseAddr, Size, VdhId) \
	(g_vdm_hal_fun_ptr.pfun_VDMHAL_OpenHAL? \
		g_vdm_hal_fun_ptr.pfun_VDMHAL_OpenHAL(MemBaseAddr, Size, VdhId): \
		VDMHAL_NULL_FUN_PTR)

/*arranging memory should be dealed with carefully, for example 4 mb 1d*/
#define VDMHAL_ArrangeMem( MemAddr, MemSize, Width, Height, PmvNum, FrameNum, UserDec, ChanID, pVdmMemArrange ) \
	(g_vdm_hal_fun_ptr.pfun_VDMHAL_ArrangeMem? \
		g_vdm_hal_fun_ptr.pfun_VDMHAL_ArrangeMem( MemAddr, MemSize, Width, Height, PmvNum, FrameNum, UserDec, ChanID, pVdmMemArrange ):\
		VDMHAL_NULL_FUN_PTR)

#define VDMHAL_CheckInt(VdhId) \
	(g_vdm_hal_fun_ptr.pfun_VDMHAL_CheckInt? \
		g_vdm_hal_fun_ptr.pfun_VDMHAL_CheckInt(VdhId): \
		VDMHAL_FUN_FALSE)
