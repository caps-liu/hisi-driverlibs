...
HI_S32 SCI_ProcWrite(struct file * file,
...
static int  sci_pm_suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    int ret;
    ...
    up(&g_SciMutex);
    HI_FATAL_SCI("sci_pm_suspend ok \n");
    return HI_SUCCESS;
}

static int  sci_pm_resume(PM_BASEDEV_S *pdev)
{
    int ret;
