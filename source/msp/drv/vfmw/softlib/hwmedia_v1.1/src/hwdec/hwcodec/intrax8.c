...
//  w->prediction_table=av_mallocz(s->mb_width*2*2);//two rows, 2 blocks per cannon mb
	w->prediction_table=av_malloc_hw(s->avctx,s->mb_width*2*2);
	if(NULL == w->prediction_table)
	{
		s->avctx->iMallocFailed = 1;
		return ;
	}
	memset(w->prediction_table,0,sizeof(s->mb_width*2*2));
...
/* 2010/08/31 11:30:00 liuxw+00139685 [AZ1D02297] */
/* 由于前面将动态分配的空间全部转换为表态组， 所以不需要释放函数 */
#if 0
/* 2010/06/18 15:30:00 liuxw+00139685 */
/* 新增函数：释放掉VLC_TABLE所占用的内存空间 */
av_cold void x8_vcl_table_free()
{
	...
	for(i=0;i<8;i++)
	{
		av_freep(&j_ac_vlc[0][0][i].table);
		av_freep(&j_ac_vlc[0][1][i].table);
		av_freep(&j_ac_vlc[1][0][i].table);
		av_freep(&j_ac_vlc[1][1][i].table);

		av_freep(&j_dc_vlc[0][i].table);
		av_freep(&j_dc_vlc[1][i].table);	
	}

	memset(j_ac_vlc,0,sizeof(j_ac_vlc));
	memset(j_dc_vlc,0,sizeof(j_dc_vlc));

	for(i=0;i<2;i++)
	{
		av_freep(&j_orient_vlc[0][i].table);
	}

	for(i=0;i<4;i++)
	{
		av_freep(&j_orient_vlc[1][i].table);
	}

	memset(j_orient_vlc,0,sizeof(j_orient_vlc));
}
#endif
/**
