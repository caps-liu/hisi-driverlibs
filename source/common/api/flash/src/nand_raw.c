...
int nand_raw_init(void)
{
...
#ifdef NAND_RAW_DBG
        PRINTF_CA("Can't find mtd device at /dev/mtdx.\n");
#endif
        return -1;
    }
    ...
    {
        PRINTF_CA("partition maybe more than %d, please increase MAX_MTD_PARTITION.\n", MAX_MTD_PARTITION);
    }
    ...
    for (ix = 0; ix < max_partition; ix++)
    {
        ...
        if ((dev = open(buf, O_RDWR)) == -1)
        {
            ...
            {
                //PRINTF_CA("%s->%d Can't open \"%s\", set to access none!\n", __func__,__LINE__,buf);
        ...
        {
            PRINTF_CA("Can't get \"%s\" information.\n", buf);
            close(dev);
            continue;
        }
    ...
    {
//        PRINTF_CA("Can't find nand type mtd device at /dev/mtdx\n");
        free(nandraw_ctrl);
        nandraw_ctrl = NULL;
        return 0;
    }
    ...
    int                 skip_badblock)
{
    ...
    {
        PRINTF_CA("startaddr should be align with pagesize(0x%lX)\n",
    ...
    unsigned long long limit_leng)
{
    ...
    {
        PRINTF_CA("offset or length should be alignment with blocksize(0x%X)\n",
    ...
    for (ix = 0; ix < nandraw_ctrl->num_partition && length; ix++)
    {
        ...
        if (ptn->readonly)
        {
            PRINTF_CA("erase a read only partition \"%s\".\n", ptn->mtddev);
            return -1;
        }
        ...
        while ((ptn->start <= offset) && (offset < ptn->end) && length && (ptn->perm & ACCESS_WR) && ptn->fd != INVALID_FD)
        {
            ...
            if (!rel)
            {
                ...
                rel = 1;
                length -= (unsigned long)((HI_U32)rel << nandraw_ctrl->blockshift);
                totalerase += (int)((HI_U32)rel << nandraw_ctrl->blockshift);
            }
...
int nand_raw_force_erase(unsigned long long offset)
{
    ...
    if ((unsigned long)offset & nandraw_ctrl->blockmask)
    {
        PRINTF_CA("offset should be alignment with blocksize(0x%X)\n",
            (unsigned int)nandraw_ctrl->blocksize);
        return -1;
    }
...
int nand_mark_badblock(unsigned long long offset, unsigned long long length)
{
    ...
    for (ix = 0; ix < nandraw_ctrl->num_partition && length; ix++)
    {
        ...
        while ((ptn->start <= offset) && (offset < ptn->end) && length && ptn->fd != INVALID_FD)
        {
            if (ptn->readonly)
            {
                PRINTF_CA("mark bad block error, a read only partition \"%s\".\n", ptn->mtddev);
                return -1;
            }
            ...
            {
                PRINTF_CA("Mark bad block 0x%llX failed!\n", offset);
            }
...
int nand_show_badblock(unsigned long long offset, unsigned long long length)
{
    ...
    for (ix = 0; ix < nandraw_ctrl->num_partition && length; ix++)
    {
        ...
        while ((ptn->start <= offset) && (offset < ptn->end) && length && ptn->fd != INVALID_FD)
        {
            ...
            if (badblock == 1)
            {
                PRINTF_CA("Bad block at address: 0x%lX of \"%s\", absolute address: 0x%llX\n",
...
int nand_raw_info(struct mtd_info_user *mtdinfo)
{
    ...
    {
        PRINTF_CA("nand flash size out of range of an unsigned long.\n");
    }
...
int nand_raw_dump_partition(void)
{
    ...
    PRINTF_CA("-------------------------\n");
    PRINTF_CA("mtd device   start length mode\n");
    for (ix = 0; ix < nandraw_ctrl->num_partition; ix++)
    {
        ptn = &nandraw_ctrl->partition[ix];

        PRINTF_CA("%-12s ", ptn->mtddev);
        PRINTF_CA("%5s ", int_to_size(ptn->start));
        PRINTF_CA("%6s ", int_to_size(ptn->end + 1 - ptn->start));
        PRINTF_CA("%2s ", ptn->readonly ? "r" : "rw");
        PRINTF_CA("\n");
    }
    ...
    int blocksize)
{
    ...
    do
    {
        ...
        if (badblock == 1)
        {
            PRINTF_CA("Skip bad block at address: 0x%llX of \"%s\", absolute address: 0x%llX\n",
...
//PRINTF_CA("\t> ptn->start=%#llx,end=%#llx,endaddr=%#llx,offset_addr=%#llx, offset=%#x, ptn->fd=%d\n",
...
int nand_raw_get_physical_index(unsigned long long startaddr, int *blockindex, int blocksize)
{
    ...
    for (ix = 0; i < logcial_index && ix < nandraw_ctrl->num_partition; ix++)
    {
        ...
        while ((i < logcial_index) && (offset_addr < ptn->end))
        {
            ...
            if (0 == badblock)
            {
                //PRINTF_CA("> Skip bad block at address: 0x%lX of \"%s\", absolute address: 0x%llX\n",
