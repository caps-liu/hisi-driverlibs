...
static inline void ff_h264_weight_WxH_mmx2(uint8_t *dst, int stride, int log2_denom, int weight, int offset, int w, int h)
{
    ...
    __asm__ volatile(
        ...
        "pshufw  $0, %%mm3, %%mm3 \n\t"
        "pxor    %%mm7, %%mm7     \n\t"
        :: "g"(weight), "g"(offset), "g"(log2_denom), "g"(tmp)
    );
    for(y=0; y<h; y+=2){
        for(x=0; x<w; x+=4){
            __asm__ volatile(
    ...
    for(y=0; y<h; y++)
    {
        for(x=0; x<w; x+=2)
        {
	        __asm__ volatile(
		        ...
		        "movd      %%mm0, %1    \n\t"   
		        "movw      %1, %0    \n\t" 
	                : "+m"(*(unsigned short *)(dst+x)), "+r"(tmp)
	                :  "m"(*(unsigned short*)(src+x))
	            );
        }
        src += stride;
        dst += stride;
...
/* 新增函数：用sse4来实现显示加权(算术运算是32位有符号操作) */
...
static inline void ff_h264_weight_WxH_sse4(uint8_t *dst, int stride, int log2_denom, int weight, int offset, int w, int h)
{
    int x, y;
    offset <<= log2_denom;
    offset += (1 << log2_denom) >> 1;
...
static inline void ff_h264_biweight_WxH_sse4(uint8_t *dst, uint8_t *src, int stride, int log2_denom, int weightd, int weights, int offset, int w, int h)
{
    int x, y;
    offset = ((offset + 1) | 1) << log2_denom;
        __asm__ volatile(
...
#define H264_WEIGHT(W,H) \
static void ff_h264_biweight_ ## W ## x ## H ## _sse4(uint8_t *dst, uint8_t *src, int stride, int log2_denom, int weightd, int weights, int offset){ \
    ff_h264_biweight_WxH_sse4(dst, src, stride, log2_denom, weightd, weights, offset, W, H); \
} \
static void ff_h264_weight_ ## W ## x ## H ## _sse4(uint8_t *dst, int stride, int log2_denom, int weight, int offset){ \
    ff_h264_weight_WxH_sse4(dst, stride, log2_denom, weight, offset, W, H); \
}
