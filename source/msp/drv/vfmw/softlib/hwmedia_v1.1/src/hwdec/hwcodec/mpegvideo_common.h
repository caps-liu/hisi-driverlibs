...
#include "dsputil.h"
#include "mpegvideo.h"
//#include "mjpegenc.h"  /* removed by lxw */
#include "msmpeg4.h"   /* removed by lxw */
//#include "faandct.h"   /* removed by lxw */
                               ...
                               uint8_t **ref_picture)
{
	...
	{
        if( (unsigned)src_x >= (unsigned)(s->h_edge_pos - 17)
           || (unsigned)src_y >= (unsigned)(s->v_edge_pos - 17))
		{
    ...
    if(s->flags&CODEC_FLAG_EMU_EDGE){
        if( (unsigned)src_x >= (unsigned)((s->h_edge_pos>>1) - 9)
           || (unsigned)src_y >= (unsigned)((s->v_edge_pos>>1) - 9))
		{
                                  ...
                                  int motion_x, int motion_y)
{
    ...
    if(s->unrestricted_mv && (s->flags&CODEC_FLAG_EMU_EDGE)){
        if(   (unsigned)src_x > (unsigned)(h_edge_pos - (motion_x&1) - w)
           || (unsigned)src_y > (unsigned)(v_edge_pos - (motion_y&1) - h))
		{
                 ...
                 int motion_x, int motion_y, int h, int is_mpeg12)
{
	...
	//guoshan + 00101841 20100421：MPEG-4如果进行了edge_mc的拷贝，内存边界无意义。
	int iEgdeMcFlag = 0;
...
if(s->quarter_sample)
{
	...
	/*xiongfei 增加判断 ref_picture所指向地址是否有效 20100401*/
    ...
    if( (unsigned) src_x > (unsigned)(s->h_edge_pos - (motion_x&1) - 16)
		|| (unsigned)src_y > (unsigned)(v_edge_pos - (motion_y&1) - h))
	{
			...
			//guoshan + 00101841 20100421：MPEG-4如果进行了edge_mc的拷贝,标志位置1。
			iEgdeMcFlag = 1;
	...
	{
		int align;
		if(( NULL == dest_y ) || (NULL == ptr_y))
		{
			av_log(s->avctx,AV_LOG_WARNING,
				"MPEG ref picture  wrong, ptr_y = %p, dest_y = %p\n", ptr_y, dest_y);
			IMEDIA_SET_ERR_MB(s->avctx->iErrorCode, IMEDIA_ERR_MB_REF);
			s->avctx->iTotalError ++;
			return;
		}
		{	
			//guoshan +00101841 20100421 ptr_y = s->edge_emu_buffer切换后下述判断无意义
			if(!iEgdeMcFlag)
			{
				...
				{
					av_log(s->avctx,AV_LOG_WARNING,
						"MPEG ref picture  wrong, ptr_y = %p, ptr = %p, ref_picture = %p\n", ptr_y, ptr, ref_picture[0]);
					IMEDIA_SET_ERR_MB(s->avctx->iErrorCode, IMEDIA_ERR_MB_REF);
					s->avctx->iTotalError ++;
					return;
				}
				...
				if(ptr_y+linesize*h > (ptr+(s->width + 32 ) * (s->height + 32 ) - align))
				{
					av_log(s->avctx,AV_LOG_WARNING,
						"MPEG ref picture  wrong\n");
					IMEDIA_SET_ERR_MB(s->avctx->iErrorCode, IMEDIA_ERR_MB_REF);
    ...
    if(!CONFIG_GRAY || !(s->flags&CODEC_FLAG_GRAY))
	{
		/*xiongfei 添加 用来判断地址空间是否有效 20100401*/
		...
		{	
			...
			//guoshan +00101841 20100421 ptr_cb = uvbuf切换后下述判断无意义
			if(!iEgdeMcFlag)
			{
				if(ptr_cb < ptr)
				{
				...
				if(((uint32_t)ptr_cb+uvlinesize*(h >> s->chroma_y_shift)) > ((uint32_t)ptr+uvlinesize*((s->height + 32) >> s->chroma_y_shift)-align))
				{
					av_log(s->avctx,AV_LOG_WARNING,
						"MPEG ref picture  wrong\n");
					IMEDIA_SET_ERR_MB(s->avctx->iErrorCode, IMEDIA_ERR_MB_REF);
       ...
       pix_op[s->chroma_x_shift][uvdxy]
		...
		{			
			uint8_t *ptr = (s->iMvFlag == MV_DIR_FORWARD)?s->last_picture.data[2]:s->next_picture.data[2];
			int align = ALIGN((s->current_picture.linesize[2]*EDGE_WIDTH>>s->chroma_x_shift) + (EDGE_WIDTH>>s->chroma_y_shift), STRIDE_ALIGN);
			if(!iEgdeMcFlag)
			{
				if(ptr_cr < ptr)
				{
				...
				if(((uint32_t)ptr_cr+uvlinesize*(h >> s->chroma_y_shift)) > ((uint32_t)ptr+uvlinesize*((s->height + 32) >> s->chroma_y_shift)- align))
				{
					av_log(s->avctx,AV_LOG_WARNING,
						"MPEG ref picture  wrong\n");
					IMEDIA_SET_ERR_MB(s->avctx->iErrorCode, IMEDIA_ERR_MB_REF);
                               ...
                               int motion_x, int motion_y, int h)
{
	...
	///* 增加对Y/U/V指针合法性的范围，避免在误码或丢包的情况下死机(avoid segment) */
	if(NULL == ref_picture[0] || NULL == ref_picture[1] || NULL == ref_picture[2])
	{
    ...
    if(  (unsigned)src_x > (unsigned)(s->h_edge_pos - (motion_x&3) - 16)
       || (unsigned)src_y > (unsigned)(v_edge_pos - (motion_y&3) - h))
	{
        ff_emulated_edge_mc(s->edge_emu_buffer, ptr_y, s->linesize,
...
static inline void chroma_4mv_motion(MpegEncContext *s,
    ...
    if(s->flags&CODEC_FLAG_EMU_EDGE)
	{
        if(   (unsigned)src_x > (unsigned)((s->h_edge_pos>>1) - (dxy &1) - 8)
           || (unsigned)src_y > (unsigned)((s->v_edge_pos>>1) - (dxy>>1) - 8))
		{
                              ...
                              qpel_mc_func (*qpix_op)[16], int is_mpeg12)
{
    ...
    if(!is_mpeg12 && s->obmc && s->pict_type != FF_B_TYPE)
	{
		...
		/*mv_cache[column][row][mvx/mvy], column取值[0,3], row取值[0,3]*/
        int16_t mv_cache[4][4][2]; 
        const int xy= s->mb_x + s->mb_y*s->mb_stride;
        const int mot_stride= s->b8_stride;
        const int mot_xy= mb_x*2 + mb_y*2*mot_stride;  //guoshan+00101841 20100504 按8x8块为单位
    ...
    case MV_TYPE_16X16:
		...
		else if(!is_mpeg12 && s->quarter_sample)
		{
			...
			//		if(s->iRet < 0) return;
			//guoshan+00101841 20100422 for test
			if(s->iRet < 0) 
			{
				av_log(s->avctx, AV_LOG_WARNING, "qpel_motion iRet = %d, return...\n", s->iRet);
				return;
			}
    ...
    if (!is_mpeg12) 
	{
        ...
        if(s->quarter_sample)
		{
            for(i=0;i<4;i++) 
			{
                ...
                if(s->flags&CODEC_FLAG_EMU_EDGE)
				{
                    if(   (unsigned)src_x > (unsigned)(s->h_edge_pos - (motion_x&3) - 8)
                       || (unsigned)src_y > (unsigned)(s->v_edge_pos - (motion_y&3) - 8))
					{
...
//	if(s->iRet < 0) return;
	...
	if(s->iRet < 0) 
	{
		av_log(s->avctx, AV_LOG_WARNING, "MPV_motion iRet = %d, return...\n", s->iRet);
        return;
	}
}
