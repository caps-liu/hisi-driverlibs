...
HI_S32 VENC_DRV_MngDequeue(queue_info_s *queue, queue_data_s *pQueue_data)
{
    ...
    if (queue->bToOMX)
    {
		ret = wait_event_interruptible_timeout(queue->wait, VENC_DRV_MngQueuePending(queue),msecs_to_jiffies(50));   // msg_pending(queue)为真则退出阻塞的等待!
		...
		list_del(&msg->list);
		memcpy(&pQueue_data->msg_info_omx, &msg->msg_info_omx,sizeof(venc_msginfo));
	...
	else
	{
		...
		msg = list_first_entry(&queue->head, queue_data_s, list);
		list_del(&msg->list);
		memcpy(&pQueue_data->queue_info, &msg->queue_info,sizeof(HI_DRV_VIDEO_FRAME_S));
		pQueue_data->bToOMX = msg->bToOMX;
		list_add(&msg->list, &queue->free);
		spin_unlock_irqrestore(&queue->msg_lock, flags);	
	}
...
HI_S32 VENC_DRV_MngQueue(queue_info_s *queue,queue_data_s *pQueue_data,HI_U32 msgcode, HI_U32 status)
{
	...
	if (queue->bToOMX)
	{
	    ...
	    if((VENC_MSG_RESP_INPUT_DONE == msgcode) || (VENC_MSG_RESP_OUTPUT_DONE == msgcode))  
	    {
			 ...
			 memcpy(&msg->msg_info_omx.buf, &pQueue_data->msg_info_omx.buf, sizeof(venc_user_buf));
		} 
	}
	else
	{
	   ...
	   memcpy(&msg->queue_info, &pQueue_data->queue_info, sizeof(HI_DRV_VIDEO_FRAME_S));
	}
