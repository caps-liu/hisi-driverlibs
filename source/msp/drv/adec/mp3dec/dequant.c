 ...
 **************************************************************************************/
static int BlockDequantize(int *inbuf, int *outbuf, int num, int scale)
{
	int tab4[4];
 ...
 **************************************************************************************/
static int ChannelDequant(int *sampleBuf, int *workBuf, int *nonZeroBound, FrameHeader *fh, 
                          SideInfoSub *sis,	ScaleFactorInfoSub *sfis, CriticalBandInfo *cbi)
{
	int i, j, w, cb;
	 ...
	 *  (BlockDequantize() does 0.25 * gainI so knocking it down by two is the same as 
	...
	for (cb = 0; cb < cbEndL; cb++) {
		...
		nonZero |= BlockDequantize(sampleBuf + i, sampleBuf + i, nSamps, gainI);
		i += nSamps;
	...
	for (cb = cbStartS; cb < cbEndS; cb++) {
		...
		for (w = 0; w < 3; w++) {
			...
			nonZero |= BlockDequantize(sampleBuf + i + nSamps*w, workBuf + nSamps*w, nSamps, gainI);
...
int Dequantize(MP3DecInfo *mp3DecInfo, int gr)
{
	...
	for (ch = 0; ch < mp3DecInfo->nChans; ch++) {
		hi->gb[ch] = ChannelDequant(hi->huffDecBuf[ch], di->workBuf, &hi->nonZeroBound[ch], fh, 
