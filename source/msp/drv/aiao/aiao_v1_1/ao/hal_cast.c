...
typedef struct
{
    HI_HANDLE hCast[AIAO_CAST_BUTT];
...
HI_S32  HAL_CAST_Create(AIAO_CAST_ID_E *penCAST, AIAO_CAST_ATTR_S *pstAttr)
{
    ...
    g_CastRm.hCast[enCAST] = (HI_HANDLE)state;
...
HI_VOID     HAL_CAST_Destroy(AIAO_CAST_ID_E enCAST)
{
    ...
    if (HI_NULL == g_CastRm.hCast[enCAST])
    {
        return;
    }
...
#if 0
HI_S32 HAL_AIAO_P_GetStatus(AIAO_CAST_ID_E enCAST, AIAO_PORT_STAUTS_S *pstProcInfo)
{
    HI_S32 Ret = HI_FAILURE;
