                  ...
                  int dxx, int dxy, int dyx, int dyy, int shift, int r, int width, int height)
{
    ...
    for(y=0; y<h; y++){
        ...
        for(x=0; x<8; x++){ //XXX FIXME optimize
            ...
            if((unsigned)src_x < (unsigned)width){
...
/*x00141957 20101229 内核态编译时栈空间过大 */
...
/* 将op_scale1和op_scale2的定义按照协议进行了修改 liuxw+00139685 */
...
#define op_scale2(x)  dst[x] = av_clip_uint8( ((src[x]*weights + dst[x]*weightd + (1<<log2_denom)) >> (log2_denom+1)) + ((offset+1)>>1))
#define H264_WEIGHT(W,H) \
static void weight_h264_pixels ## W ## x ## H ## _c(uint8_t *block, int stride, int log2_denom, int weight, int offset){ \
    int y; \
	/* 2010/30/10 11:44:00 liuxw+00139685 */ \
	/* 将以下offset的做法全部去掉，其正确的做法见op_scale1的定义 */ \
    /* offset <<= log2_denom; */ \
...
static void biweight_h264_pixels ## W ## x ## H ## _c(uint8_t *dst, uint8_t *src, int stride, int log2_denom, int weightd, int weights, int offset){ \
    int y; \
	/* 2010/30/10 11:44:00 liuxw+00139685 */ \
	/* 将以下offset的做法全部去掉，其正确的做法见op_scale2的定义 */ \
...
static int nsse16_c(void *v, uint8_t *s1, uint8_t *s2, int stride, int h){
    ...
    for(y=0; y<h; y++)
	{
        ...
        if(y+1<h)
		{
            for(x=0; x<15; x++)
			{
                score2+= FFABS(  s1[x  ] - s1[x  +stride] - s1[x+1] + s1[x+1+stride]) -FFABS(  s2[x  ] - s2[x  +stride] - s2[x+1] + s2[x+1+stride]);
            }
        }
        s1+= stride;
        s2+= stride;
...
//          av_log(NULL, AV_LOG_ERROR,"internal error in cmp function selection\n");
			av_log(NULL, AV_LOG_WARNING,"internal error in cmp function selection\n");
        }
    }
}
...
static void add_bytes_c(uint8_t *dst, uint8_t *src, int w){
    long i;
    for(i=0; i<=(int)(w-sizeof(long)); i+=sizeof(long)){
        long a = *(long*)(src+i);
        long b = *(long*)(dst+i);
...
static int rd8x8_c(/*MpegEncContext*/ void *c, uint8_t *src1, uint8_t *src2, int stride, int h){
 ...
 #else
    uint64_t *aligned_bak=_alloca(sizeof(uint64_t)*stride);     //add by lxw
 #endif
    DCTELEM * const temp= (DCTELEM*)aligned_temp;
...
#undef dspfunc
	...
	/*x00141957 2010 1230 内核态编译栈空间过大*/
#if 0
...
#define SET_CMP_FUNC(name) \
...
#if (CONFIG_VP6_DECODER) 
	{
	 ...
	 c->vp6_filter_diag4_h	= c->vp6_filter_diag4_v = vp6_filter_hv4;
	 c->vp56_edge_filter = vp56_edge_filter_c;
	 c->vp6_filter_diag2 = vp6_filter_diag2_c;
	 c->vp6_filter_diag2_h = vp6_filter_diag2_h_c;
	 c->vp6_filter_diag2_v = vp6_filter_diag2_v_c;
	 c->idct_slow1_put = idct_slow1_put;
	 c->idct_slow3_put = idct_slow3_put;
	 c->idct_slow10_put = idct_slow10_put;
	 c->idct_slow_put = idct_slow_put;
	 c->idct_slow1_add = idct_slow1_add;
	 c->idct_slow3_add = idct_slow3_add; 
	  c->idct_slow10_add = idct_slow10_add;
	 c->idct_slow_add = idct_slow_add; 
    }
#endif
...
//      av_log(avctx, AV_LOG_ERROR, "Internal error, IDCT permutation not set\n");
		av_log(avctx, AV_LOG_WARNING, "Internal error, IDCT permutation not set\n");
    }
}
