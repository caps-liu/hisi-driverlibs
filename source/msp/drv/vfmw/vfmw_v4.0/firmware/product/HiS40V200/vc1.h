...
typedef struct
{
    ...
    VC1_ADVSLCSYNTAX_S AdvSlcSyntax[128]; //1080p just need these. max is 512 for VC1 original protocol
} VC1_VFMWSYNTAX_S;
#if 0
typedef struct {
...
typedef struct
{
    ...
    UINT32 image_stride;
    UINT32 luma_fwd_phy_addr;
    UINT32 chrom_fwd_phy_addr;
    UINT32 luma_bwd_phy_addr;
    UINT32 chrom_bwd_phy_addr;
...
typedef struct
{
    UINT32 bit_offset_0;
    UINT32 bit_len_0;
    UINT32 bit_stream_addr_0;
    UINT32 bit_offset_1;
    UINT32 bit_len_1;
    UINT32 bit_stream_addr_1;

    UINT32 SliceQP;
    UINT32 osvquant;
    UINT32 dblk_filter_passthrough;
    UINT32 first_mb_in_slice;
    UINT32 next_slice_para_addr;
...
typedef struct
{
    ...
    SINT32  YPixHeight;
    SINT32  PixScale;
    UINT32  StartAddr;  // 参考图像(帧或场)左上角像素位置
} VC1_PRCBLOCKINFO_S;
...
typedef struct
{
    ...
    UINT32  pic_addressx[3];  // pic_addressx[0]: 前向参考帧地址;   pic_addressx[1]: 后向参考帧地址
