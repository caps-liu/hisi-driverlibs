...
/*CNcomment:问题单号:九联VOIP反复呼叫SLIC偶尔无反应 by FangZhonghua, 2011/3/28 */
...
/*CNcomment:问题单号:九联VOIP反复呼叫SLIC偶尔无反应 by FangZhonghua, 2011/3/29*/
...
/* BEGIN: Modified for PN: initialize with audio error by f00172091, 2011/1/30 */
    ...
    if (1 == atomic_inc_return(&open_cnt))
    {
        ...
        if(ret != VP_STATUS_SUCCESS)
        {
			...
			/*CNcomment: 问题单号:无SLIC扣板AIAO初始化内核出错 by FangZhonghua, 2011/3/17*/
   ...
   else
   {
        ...
        switch(cmd)
        {
            case SLAC_INITIAL:
                ...
                //    hisi_slac_init();
                /* END: Deleted for PN: initialize with audio error by f00172091, 2011/1/30 */
                break;
            case SLAC_REG_READ:
                if(temp.mpireg & 0x01)
                {
...
static int __INIT__ slac_init(void)
{
    int res = 0;
    /* BEGIN: Modified for PN: initialize with audio error by f00172091, 2011/1/30 */
...
static void __EXIT__ slac_exit(void)
{
/* BEGIN: Modified for PN: initialize with audio error by f00172091, 2011/1/30 */
