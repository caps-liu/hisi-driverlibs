...
#define ATRC(x...)      printk(KERN_ALERT x)
#else
#define TRP() 
#define ATRC(fmt, ...) 
#endif
...
{
  //  printk(KERN_ALERT" suspend_post ok !\n");
...
//	printk(KERN_ALERT"resume pre ok !\n");
	return ret;
}
...
static __devinit int htd_hw_snd_dev_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &hw_snd_soc_hiaiao;
	int ret;
...
static __devexit int htd_hw_snd_dev_remove(struct platform_device *pdev)
{
...
static struct platform_driver htd_hw_sndcard_driver = {
    	.driver = {
		.name = "hisi-i2s_only-sndcard",
		.owner = THIS_MODULE,
	},
	.probe  = htd_hw_snd_dev_probe,
	.remove = __devexit_p(htd_hw_snd_dev_remove),
};
...
int hisndcard_hw_init(void)
{
    int ret = 0;
    //register device
    hisi_hw_snd_device = platform_device_alloc("hisi-i2s_only-sndcard", -1);
    ...
    //register drvier
    return platform_driver_register(&htd_hw_sndcard_driver);
}
