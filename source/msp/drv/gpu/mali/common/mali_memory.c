...
void mali_memory_session_end(struct mali_session_data *session_data)
{
	...
	while (err == _MALI_OSK_ERR_BUSY)
	{
		...
		_mali_osk_mutex_wait( session_data->memory_lock );
		...
		if (!_mali_osk_list_empty(&session_data->memory_head))
		{
			...
			_MALI_OSK_LIST_FOREACHENTRY(descriptor, temp, &session_data->memory_head, mali_memory_allocation, list)
			{
				...
				{
					_mali_osk_mutex_signal( session_data->memory_lock );
					/*
						* Reason for this;
...
{
	mali_memory_allocation * descriptor;
	_mali_osk_mutex_t *descriptor_lock;
	_mali_osk_errcode_t err;
	...
	while (err == _MALI_OSK_ERR_BUSY)
	{
		if (descriptor_lock)
		{
			_mali_osk_mutex_wait( descriptor_lock );
		}
		...
		if (descriptor_lock)
		{
			_mali_osk_mutex_signal( descriptor_lock );
		}
...
{
	_mali_osk_mutex_wait(page_table_cache.lock);
	...
	if (!_mali_osk_list_empty(&page_table_cache.partial))
	{
		...
		_mali_osk_mutex_signal(page_table_cache.lock);
...
static _mali_osk_errcode_t mali_mmu_page_table_cache_create(void)
{
	page_table_cache.lock = _mali_osk_mutex_init( _MALI_OSK_LOCKFLAG_ORDERED, _MALI_OSK_LOCK_ORDER_MEM_PT_CACHE);
...
static void mali_mmu_page_table_cache_destroy(void)
{
	...
	_mali_osk_mutex_term(page_table_cache.lock);
}
