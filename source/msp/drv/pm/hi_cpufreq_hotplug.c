...
int  hi_cpufreq_hotplug_init(void)
{
	int err;
	cputime64_t wall;
	u64 idle_time;
	...
	if (idle_time != -1ULL) {
		...
		HI_ERR_PM("cpufreq-hotplug: %s: assumes CONFIG_NO_HZ\n",
				__func__);
	...
	if (!khotplug_wq) {
		HI_ERR_PM("Creation of khotplug failed\n");		
		return -EFAULT;
	}
	
	err = cpufreq_register_governor(&cpufreq_gov_hotplug);
	if (err){
		HI_ERR_PM("cpufreq_register_governor error\n");
...
void  hi_cpufreq_hotplug_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_hotplug);
	destroy_workqueue(khotplug_wq);
}

MODULE_AUTHOR("wangjian <stand.wang@huawei.com>");
