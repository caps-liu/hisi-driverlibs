...
static HI_U32 g_u32HdmiRxI2cNum = 2;
static HI_U8 HDMIRX_FST_DEV = 0x64;

HI_U32 gpioScl = 19; //gpio2_3
HI_U32 gpioSda = 20; //gpio2_4
HI_U32 gpioRst = 16; //gpio2_0
HI_U32 gpioInt = 17; //gpio2_1
...
HI_S32 SIL9293_HAL_Init(HI_VOID)
{
    ...
    //I2C_DRV_SetRate(g_u32HdmiRxI2cNum, 100000);
    if (bLowCi2ca == 1)
    {
        HDMIRX_FST_DEV = 0x64;
    }
...
HI_VOID SIL9293_HAL_SetVideoAuto(HI_VOID)
{
    ...
    u8Value = (u8Value & (~(BIT6 | BIT7))) | 0x00;
...
HI_VOID SIL9293_HAL_GetStable(HI_BOOL *pbStable)
{
    ...
    {
#if 0
        HI_U32 u32HTotal, u32VTotal;
        SIL9293_HAL_GetHVTotal(&u32HTotal, &u32VTotal);
        HI_ERR_HDMIRX("HTotal=%d, VTotal=%d\n", u32HTotal, u32VTotal);
#endif
...
HI_VOID SIL9293_HAL_GetICLK(HI_U8 *pu8ICLK)
{
    HI_U8 u8Value;
