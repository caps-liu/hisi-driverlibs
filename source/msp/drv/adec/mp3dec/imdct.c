...
#if defined(ENA_ANTIALIAS_COEF_OPT)
static void AntiAlias(int *x, int nBfly)
{
	...
	for (k = nBfly; k > 0; k--) {
		...
		x[-1] = (MUL32_16T(c0, a0) - MUL32_16T(c1, b0)) << 1;	
		x[0] =  (MUL32_16T(c0, b0) + MUL32_16T(c1, a0)) << 1;

		a0 = x[-2];			c0 = *c;	c++;	b0 = x[1];		c1 = *c;	c++;
		x[-2] = (MUL32_16T(c0, a0) - MUL32_16T(c1, b0)) << 1;	
		x[1] =  (MUL32_16T(c0, b0) + MUL32_16T(c1, a0)) << 1;
		
		a0 = x[-3];			c0 = *c;	c++;	b0 = x[2];		c1 = *c;	c++;
		x[-3] = (MUL32_16T(c0, a0) - MUL32_16T(c1, b0)) << 1;	
		x[2] =  (MUL32_16T(c0, b0) + MUL32_16T(c1, a0)) << 1;

		a0 = x[-4];			c0 = *c;	c++;	b0 = x[3];		c1 = *c;	c++;
		x[-4] = (MUL32_16T(c0, a0) - MUL32_16T(c1, b0)) << 1;	
		x[3] =  (MUL32_16T(c0, b0) + MUL32_16T(c1, a0)) << 1;

		a0 = x[-5];			c0 = *c;	c++;	b0 = x[4];		c1 = *c;	c++;
		x[-5] = (MUL32_16T(c0, a0) - MUL32_16T(c1, b0)) << 1;	
		x[4] =  (MUL32_16T(c0, b0) + MUL32_16T(c1, a0)) << 1;

		a0 = x[-6];			c0 = *c;	c++;	b0 = x[5];		c1 = *c;	c++;
		x[-6] = (MUL32_16T(c0, a0) - MUL32_16T(c1, b0)) << 1;	
		x[5] =  (MUL32_16T(c0, b0) + MUL32_16T(c1, a0)) << 1;

		a0 = x[-7];			c0 = *c;	c++;	b0 = x[6];		c1 = *c;	c++;
		x[-7] = (MUL32_16T(c0, a0) - MUL32_16T(c1, b0)) << 1;	
		x[6] =  (MUL32_16T(c0, b0) + MUL32_16T(c1, a0)) << 1;

		a0 = x[-8];			c0 = *c;	c++;	b0 = x[7];		c1 = *c;	c++;
		x[-8] = (MUL32_16T(c0, a0) - MUL32_16T(c1, b0)) << 1;	
		x[7] =  (MUL32_16T(c0, b0) + MUL32_16T(c1, a0)) << 1;
	}
}
 ...
 **************************************************************************************/
#if defined ENA_WINPREVIOUS_COEF_OPT

static void WinOverlap(int *xPrev, int *xPrevWin, int btPrev)
{
	...
	if (btPrev == 2) {
		...
		xPrevWin[ 0] = MUL32_16T(wpLo[ 6], xPrev[2]) + MUL32_16T(wpLo[0], xPrev[6]);
		xPrevWin[ 1] = MUL32_16T(wpLo[ 7], xPrev[1]) + MUL32_16T(wpLo[1], xPrev[7]);
		xPrevWin[ 2] = MUL32_16T(wpLo[ 8], xPrev[0]) + MUL32_16T(wpLo[2], xPrev[8]);
		xPrevWin[ 3] = MUL32_16T(wpLo[ 9], xPrev[0]) + MUL32_16T(wpLo[3], xPrev[8]);
		xPrevWin[ 4] = MUL32_16T(wpLo[10], xPrev[1]) + MUL32_16T(wpLo[4], xPrev[7]);
		xPrevWin[ 5] = MUL32_16T(wpLo[11], xPrev[2]) + MUL32_16T(wpLo[5], xPrev[6]);
		xPrevWin[ 6] = MUL32_16T(wpLo[ 6], xPrev[5]);
		xPrevWin[ 7] = MUL32_16T(wpLo[ 7], xPrev[4]);
		xPrevWin[ 8] = MUL32_16T(wpLo[ 8], xPrev[3]);
		xPrevWin[ 9] = MUL32_16T(wpLo[ 9], xPrev[3]);
		xPrevWin[10] = MUL32_16T(wpLo[10], xPrev[4]);
		xPrevWin[11] = MUL32_16T(wpLo[11], xPrev[5]);
		...
		for (i = 9; i > 0; i--) {
			...
			*xpwLo++ = MUL32_16T(wLo, x);
			*xpwHi-- = MUL32_16T(wHi, x);
		}
	}
}
...
#endif  /* ENA_WINPREVIOUS_COEF_OPT */

/**************************************************************************************
 * Function:    InvertFrequency
 *
 ...
 **************************************************************************************/
static int InvertFrequency(int *y, int *xPrev, int blockIdx, int es)
{
	int i, d, mOut;
...
#if defined ENA_IDCT9_COEF_OPT
    m1 =  MUL32_16T(c9_0, x3);
	m3 =  MUL32_16T(c9_0, a10);
	m5 =  MUL32_16T(c9_1, a5);
	m6 =  MUL32_16T(c9_2, a6);
	m7 =  MUL32_16T(c9_1, a8);
	m8 =  MUL32_16T(c9_2, a5);
	m9 =  MUL32_16T(c9_3, a9);
	m10 = MUL32_16T(c9_4, a7);
	m11 = MUL32_16T(c9_3, a3);
	m12 = MUL32_16T(c9_4, a9);
 ...
 *                only save 9 xPrev samples, using symmetry (see WinOverlap())
 *
...
#if defined ENA_IMDCT36_COEF_OPT
...
static int IMDCT36(int *xCurr, int *xPrev, int *y, int btCurr, int btPrev, int blockIdx, int gb)
{
	...
	if (btPrev == 0 && btCurr == 0) {
		...
		for (i = 0; i < 9; i++) {
			...
			xo = MUL32_16T(c, xo);			/* 2*c18*xOdd (mul by 2 implicit in scaling)  */
			xe >>= 2;
            ...
            yLo = (d + (MUL32_16T(*wp++, t) << 2));
			yHi = (s + (MUL32_16T(*wp++, t) << 2));
		 ...
		 * output xPrevWin has at least 3 guard bits (xPrev has 2, gain 1 in WinOverlap)
		 */
		WinOverlap(xPrev, xPrevWin, btPrev);
		...
		for (i = 0; i < 9; i++) {
			...
			yLo = (xPrevWin[i]    + MUL32_16T(wp[i], d)) << 2;
			yHi = (xPrevWin[17-i] + MUL32_16T(wp[17-i], d)) << 2;
	...
	xPrev -= 9;
	mOut |= InvertFrequency(y, xPrev, blockIdx, es);
...
static int IMDCT36(int *xCurr, int *xPrev, int *y, int btCurr, int btPrev, int blockIdx, int gb)
{
	...
	if (btPrev == 0 && btCurr == 0) {
		...
		for (i = 0; i < 9; i++) {
            ...
            yLo = (d + (MULSHIFT32(*wp++, t) << 2));
			yHi = (s + (MULSHIFT32(*wp++, t) << 2));
		...
		for (i = 0; i < 9; i++) {
			...
			yLo = (xPrevWin[i]    + MULSHIFT32(wp[i], d)) << 2;
			yHi = (xPrevWin[17-i] + MULSHIFT32(wp[17-i], d)) << 2;
 ...
 */
#if defined ENA_IMDCT12_COEF_OPT
...
static __inline void imdct12 (int *x, int *out)
{
	...
	a0 = MUL32_16T(c3_0, x3) << 1;
	a1 = x1 + (x5 >> 1);
	a2 = x1 - x5;

	/* cos window odd samples, mul by 2, eat sign bit */
	x1 = MUL32_16T(c6[0], a1 + a0) << 2;			
	x3 = MUL32_16T(c6[1], a2) << 2;
	x5 = MUL32_16T(c6[2], a1 - a0) << 2;
...
#if defined ENA_IMDCT12x3_COEF_OPT
		for (i = 0; i < 3; i++) {
		yLo = (xPrevWin[ 0+i] << 2);
		mOut |= FASTABS(yLo);	y[( 0+i)*NBANDS] = yLo;
		yLo = (xPrevWin[ 3+i] << 2);
		mOut |= FASTABS(yLo);	y[( 3+i)*NBANDS] = yLo;
		yLo = (xPrevWin[ 6+i] << 2) + (MUL32_16T(wp[0+i], xBuf[3+i]));	
		mOut |= FASTABS(yLo);	y[( 6+i)*NBANDS] = yLo;
		yLo = (xPrevWin[ 9+i] << 2) + (MUL32_16T(wp[3+i], xBuf[5-i]));	
		mOut |= FASTABS(yLo);	y[( 9+i)*NBANDS] = yLo;
		yLo = (xPrevWin[12+i] << 2) + (MUL32_16T(wp[6+i], xBuf[2-i]) + MUL32_16T(wp[0+i], xBuf[(6+3)+i]));	
		mOut |= FASTABS(yLo);	y[(12+i)*NBANDS] = yLo;
		yLo = (xPrevWin[15+i] << 2) + (MUL32_16T(wp[9+i], xBuf[0+i]) + MUL32_16T(wp[3+i], xBuf[(6+5)-i]));	
 ...
 **************************************************************************************/
static int HybridTransform(int *xCurr, int *xPrev, int y[MP3_BLOCK_SIZE][NBANDS], SideInfoSub *sis, BlockCount *bc)
{
...
*/
    if ( (bc->nBlocksLong > NBANDS) || (bc->nBlocksTotal > NBANDS) || (bc->nBlocksPrev > NBANDS) )
    {
        return -1;
    }
	...
	for (   ; i < bc->nBlocksPrev; i++) {
		...
		if (i < bc->prevWinSwitch)
			 prevWinIdx = 0;
		WinOverlap(xPrev, xPrevWin, prevWinIdx);
...
int IMDCT(MP3DecInfo *mp3DecInfo, int gr, int ch)
{
  ...
  /*ASSERT(hi->nonZeroBound[ch] <= MAX_NSAMP);*/
    if (hi->nonZeroBound[ch] > MAX_NSAMP)
    {
        return -1;
    }
  ...
  /*ASSERT(mi->numPrevIMDCT[ch] <= NBANDS);*/
    if ( (mi->numPrevIMDCT[ch] > NBANDS) || (mi->numPrevIMDCT[ch] < 0) )
    {
        return -1;
    }
