...
#if 0
    printk("pstCb->pu32Read = 0x%x, ", *(pstCb->pu32Read));
    printk("pstCb->pu32Write = 0x%x, ", *(pstCb->pu32Write));
    printk("pstCb->pu8Data = 0x%x, ", *(pstCb->pu8Data));
...
static inline HI_U32 CIRC_BUF_Write(CIRC_BUF_S *pstCb, HI_U8 *pDest, HI_U32 u32Len)
{
    ...
    {
        pVirAddr = (HI_U32 *)((HI_U32)pstCb->pu8Data + u32WtPos[i]);
        if(pDest)  
        {
