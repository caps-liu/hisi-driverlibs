...
HI_S32 HI_MPI_DISP_Init(HI_VOID)
{
    ...
    if (HI_FAILURE == stat(g_DispDevName, &st))
    {
        HI_FATAL_DISP("DISP is not exist.\n");
        HI_DISP_UNLOCK();
        return HI_ERR_DISP_DEV_NOT_EXIST;
    }

    if (!S_ISCHR (st.st_mode))
    {
        HI_FATAL_DISP("DISP is not device.\n");
        HI_DISP_UNLOCK();
        return HI_ERR_DISP_NOT_DEV_FILE;
    }
...
HI_S32 HI_MPI_DISP_SetTiming(HI_DRV_DISPLAY_E enDisp,  HI_DRV_DISP_TIMING_S *pstTiming)
{
    ...
    {
        HI_ERR_DISP("para pstLcdPara->IDV is invalid.\n");
        return HI_ERR_DISP_INVALID_PARA;
    }
    ...
    {
        HI_ERR_DISP("para pstLcdPara->IHS is invalid.\n");
        return HI_ERR_DISP_INVALID_PARA;
    }
    ...
    {
        HI_ERR_DISP("para pstLcdPara->IVS is invalid.\n");
        return HI_ERR_DISP_INVALID_PARA;
    }
    ...
    {
        HI_ERR_DISP("para pstLcdPara->ClockReversal is invalid.\n");
        return HI_ERR_DISP_INVALID_PARA;
    }

    if (pstTiming->u32DataWidth > 24)
    {
        HI_ERR_DISP("para pstLcdPara->DataWidth is invalid.\n");
        return HI_ERR_DISP_INVALID_PARA;
    }

    if (pstTiming->eDataFmt >= HI_DRV_DISP_INTF_DATA_FMT_BUTT)
    {
        HI_ERR_DISP("para pstLcdPara->ItfFormat is invalid.\n");
        return HI_ERR_DISP_INVALID_PARA;
    }
...
#if 0
        printf(" CMD_DISP_ACQUIRE_CAST_FRAME w=%d, h=%d,y=0x%x,c=0x%x\n", 
