...
/*xiongfei 添加此头文件 xiongfei20100212*/
#include "mpegvideo_parser.h"
/*错误码头文件 xiongfei20100303*/
                          ...
                          DCTELEM block[12][64])
{
	...
	/*s->chroma_format指示是否420 422 444 xiongfei20100222*/
...
//            assert(s->current_picture.mb_type[ s->mb_x + s->mb_y*s->mb_stride - 1]&(MB_TYPE_16x16|MB_TYPE_16x8));
	...
	/*以下解码宏块类型 xiongfei20100222*/
    switch(s->pict_type) {
    default:
    case FF_I_TYPE:
		/*I帧宏块模式解码参考B-2 xiongfei20100222*/
        if (get_bits1(&s->gb) == 0) {
			/*增加错误码 xiongfei20100317*/
            if (get_bits1(&s->gb) == 0){
        ...
        break;
    case FF_P_TYPE:
		/*P帧宏块模式解码参考B-3 xiongfei20100222*/
        ...
        break;
    case FF_B_TYPE:
		/*B帧宏块模式解码参考B-4 xiongfei20100222*/
...
//    motion_type = 0; /* avoid warning */
    ...
    if (IS_INTRA(mb_type)) {
		/*intra块解码流程 xiongfei20100222*/
		/*将参数地址开始6个block大小空间进行清零 xiongfei20100224*/
...
*/
		/*intra块vld ，每次一个block，解码出来数据存放在s->pblocks[i]中xiongfei20100222*/
		...
		/*inter以及B解码流程 xiongfei20100222*/
        if (mb_type & MB_TYPE_ZERO_MV){
			/*0mv的情况 xiongfei20100222*/
			...
			/*omv的情况下，重置mv预测值 xiongfei20100222*/
            s->last_mv[0][0][0] = 0;
            s->last_mv[0][0][1] = 0;
            s->last_mv[0][1][0] = 0;
            s->last_mv[0][1][1] = 0;
...
//FIXME decide if MBs in field pictures are MB_TYPE_INTERLACED
			...
			/*根据运动矢量类型进行解析mv xiongfei20100222*/
			/*此时所解析出mv仅仅是根据码流解析出来数据，没有进行任何变换 xiongfei20100224*/
            switch(motion_type) {
                ...
                if (s->picture_structure == PICT_FRAME) {
                    ...
                    for(i=0;i<2;i++) {
                        if (USES_LIST(mb_type, i)) {
							/*xiongfei20100401 modified 检查fcode*/
							{
		...
		/*解码inter或者B块残差部分 xiongfei20100222*/
        s->mb_intra = 0;
        if (HAS_CBP(mb_type)) {
            s->dsp.clear_blocks(s->block[0]);
...
static int mpeg_decode_motion(MpegEncContext *s, int fcode, int pred)
{
	...
	/*xiongfei20100401修改此函数  增加对low high的判断 */
//	int low,high,range;
	...
	/*不打算在此增加错误码 但是增加打印信息 提示此处比特错误 xiongfei20100317*/
    if (code < 0) {
                               ...
                               int n)
{
...

    {
        ...
        // special case for first coefficient, no need to add second VLC table
		/*非intra块的第一个dct系数分开解码，请参照协议 xiongfei20100224 */
        UPDATE_CACHE(re, &s->gb);
        if (((int32_t)GET_CACHE(re, &s->gb)) < 0) {
            level= (3*qscale*quant_matrix[0])>>5;
            if(GET_CACHE(re, &s->gb)&0x40000000)
                               ...
                               int n)
{
	...
	/*此处获得本block DC值 不需要反量化 xiongfei20100223*/
    dc = s->last_dc[component];
    dc += diff;

    s->last_dc[component] = dc;

	/*此处求出当前intra块的DC系数值 xiongfei20100223*/
	...
	/*开始intra块的vlc解码 xiongfei20100224*/
    {
        OPEN_READER(re, &s->gb);
        /* now quantify & encode AC coefficients */
        for(;;) {
...
/*将 Mpeg1Context的定义移到mpeg12.h头文件中 xiongfei20100221*/
static av_cold int mpeg_decode_init(AVCodecContext *avctx)
{
	...
	/* xiongfei20100302*/
	/*parse_context.buffe中的缓冲空间在此初始化，避免了在ff_combine_frame函数中重新分配内存 xiongfei20100302*/
	/* 新增代码（l00139685): 给rbsp_buffer和parse.buffer分配内存 */
...
static int mpeg_decode_postinit(AVCodecContext *avctx){
	...
	/*x00141957 20100519 修改 屏蔽对aspect_ratio_info的判断，因为不影响解码*/
	/*问题单号：AZ1D02088
	修改人：熊飞 +00141957
	时间：2010/5/19
	问题描述： 对aspect_ratio_info的判断引起内外不一致，从而导致解码段错误
	问题修改： 不对aspect_ratio_info的错误做严格检查*/
    if (
...
//           avctx->sample_aspect_ratio= av_d2q(1.0/ff_mpeg1_aspect[s->aspect_ratio_info], 255);
  ...
  /*          av_reduce(&s->avctx->time_base.den,&s->avctx->time_base.num,
		...
		/* 将mpeg2 idct的算法改成与libmpeg2库一致，提升质量且便于做比特一致性测试 */
/*
#if defined(__GNUC__)
                                ...
                                const uint8_t *buf, int buf_size)
{
	...
	/*MpegEncContext结构体中一些内存初始化在此 xiongfei20100222*/
	/*增加打印错误日志并添加错误码 xiongfei20100317*/
    ...
    if (s->pict_type == FF_P_TYPE || s->pict_type == FF_B_TYPE) {
		...
		/*对full_pel_forward_vector和forward_f_code根据协议做检测 x00141957 20100505*/
...
//            return -1;
    ...
    if (s->pict_type == FF_B_TYPE) {
        ...
        f_code = get_bits(&s->gb, 3);
		/*对full_pel_backward_vector和backward_f_code根据协议做检测 x00141957 20100505*/
...
static void mpeg_decode_sequence_extension(Mpeg1Context *s1)
{
	...
	/*需要对以下两者进行检查 x00141957 20100506*/
    s->progressive_sequence = get_bits1(&s->gb); /* progressive_sequence */
    s->chroma_format = get_bits(&s->gb, 2); /* chroma_format 1=420, 2=422, 3=444 */
	/*对s->chroma_format进行检查 x00141957 20100507*/
...
//  s->height |= (vert_size_ext << 12);
	...
	/*以下五行代码为新增 ，因为mpeg2宽高等信息都只在序列头中给出，所以实际宽高值只能在这里得到 获得通道实际解码参数xiongfei20100303*/
	/*以下两行代码为新增 ，因为mpeg2宽高等信息都只在序列头中给出，所以实际宽高值只能在这里得到 xiongfei20100303*/
	/*考虑到我们的解码器不支持中间变换分辨率，所以以第一次所读到的图像宽高为主 xiongfei20100401*/

	/*不支持大于 1 << 12的图像 xioingfei20100401*/
...
/*不作任何检查 x00141957 20100506 跟TI保持一致*/
...
static void mpeg_decode_picture_coding_extension(Mpeg1Context *s1)
{
	...
	/*跟据协议增加对picture_structure的检测 x00141957 20100506*/
...
static int mpeg_field_start(MpegEncContext *s, const uint8_t *buf, int buf_size){
    ...
    if(s->first_field || s->picture_structure==PICT_FRAME){
		...
		/*一帧错误隐藏初始化 xiongfei20100222*/
        ff_er_frame_start(s);

        /* first check if we must repeat the frame */
		/*repeat_pict 什么意思？ xiongfei20100222*/
                             ...
                             const uint8_t **buf, int buf_size)
{
	...
	/*查看是否超过一帧 xiongfei20100222*/
	/*添加错误码 xiongfei20100317*/
	...
	/*初始化dc预测值 并且pmv值内存清零 xiongfei20100222*/
    ff_mpeg1_clean_buffers(s);

    s->interlaced_dct = 0;

	/*获得当前slice qscale xiongfei20100222*/
    s->qscale = get_qscale(s);
...
// 			return -1;	
	...
	/*初始化一些block信息以及yuv重建帧存放地址 xiongfei20100222*/
	...
	/*进入解码循环，解码slice中每一个宏块 xiongfei20100222*/
    for(;;) {
        //If 1, we memcpy blocks in xvmcvideo.
		/*xiongfei20100210*/  
		if(CONFIG_MPEG_XVMC_DECODER && s->avctx->xvmc_acceleration > 1)
        ...
        /*进入解码每一个宏块 ,此流程中只有码流解析部分 xiongfei20100222*/
		/*增加打印错误日志 xiongfei20100317*/
		...
		/*不需要进入这一步 当有错误隐藏，会引起内存写操作越界 xiongfei20100403*/
		if(0)
		...
		/*指针往后移 y:16 uv:8 xiongfei20100222*/
        s->dest[0] += 16 >> lowres;
        s->dest[1] +=(16 >> lowres) >> s->chroma_x_shift;
        s->dest[2] +=(16 >> lowres) >> s->chroma_x_shift;

		/*此处进行数据重建，其中包括mc,idct xiongfei20100222*/
        MPV_decode_mb(s, s->block);

        if (++s->mb_x >= s->mb_width) {
            const int mb_size= 16>>s->avctx->lowres;
			
			/*进入下一个宏块行 xiongfei20100222*/
                                 ...
                                 const uint8_t *buf, int buf_size)
{
	...
	/*增加打印错误日志并添加错误码 并增加返回 xioingfei20100317*/
    ...
    /*x00141957 20100505 根据协议修改做严格检查*/
	//skip_bits(&s->gb, 1);
                             ...
                             const uint8_t *buf, int buf_size)
{
	...
	/*此时已经耗尽比特数,buf_size表示缓冲中比特数目 xiongfei20100221*/
...
//         decode_chunks(avctx, picture, data_size, avctx->extradata, avctx->extradata_size);
 	if(avctx->extradata_num && !avctx->frame_number)
 	{
		...
		/*x00141957 20100511 返回值修改以保证能完全解完码流*/
	ret = decode_chunks(avctx, picture, data_size, buf, buf_size);

	/*x00141957 20100726获得当前码流的aspect ratio info*/
                             ...
                             const uint8_t *buf, int buf_size)
{
    ...
    for(;;) {
		...
		/*如果没能找到起始码，则表明此帧数据结束 xiongfei20100221*/
        if (start_code > 0x1ff){
...
//						avctx->uiDisFramesID = s2->input_picture_number + 1;
        ...
        case PICTURE_START_CODE:
			...
			/*以下为新增代码,将当前帧实际编码类型给avctx，让外面看见 xiongfei20100304*/
			{
				switch(s2->pict_type)
				{
				case FF_I_TYPE:
					avctx->uiDecIFrames++;  /* 增加已解码I帧数量 */
					break;
				case FF_P_TYPE:
					avctx->uiDecPFrames++;	 /* 增加已解码P帧数量 */
					break;
				case FF_B_TYPE:
					avctx->uiDecBFrames++;  /* 增加已解码B帧数量 */
					break;
				default:	break;
				}
			}
        ...
        case EXT_START_CODE:
			...
			/*xiongfei modified 20100331*/
			{
				...
				/*以下为 新增代码 将yuv模式付给avctx让外面看见 xiongfei201000303*/
        ...
        case GOP_START_CODE:
            ...
            break;
        default:
			/*如果找到slice头 ,则进入slice解码流程 xiongfei20100221*/
            if (start_code >= SLICE_MIN_START_CODE &&
                start_code <= SLICE_MAX_START_CODE) {
                int mb_y= start_code - SLICE_MIN_START_CODE;

				/*对各关键数据进行检查 x00141957 20100511*/
				...
				/*如果是第一个slice，说明刚开始编码一帧，初始化内存数据 ，但是也考虑到了底场第一个slice编码的情况 xiongfei20100222*/
                if(s2->first_slice)
				{
                    /*挪到后面 x00141957 20100511*/
					/*s2->first_slice=0;*/
					/*以下为新增代码，判断通道资源是否满足码流 ，主要判断宽高以及chromaformat xiongfei20100303*/
					{
...
//							avctx->iErrorCode = (avctx->usActualWidth > avctx->usSourceWidth) ? IMEDIA_CODEC_ERR_INVALID_WIDTH : IMEDIA_CODEC_ERR_INVALID_HEIGHT;
                ...
                if(avctx->thread_count > 1){
					...
					/*此处正式进入解码slice xiongfei20100222*/
                    ret = mpeg_decode_slice(s, mb_y, &buf_ptr, input_size);
                    emms_c();

					/*如果解码返回失败，则为以后的错误隐藏 xiongfei20100222*/
                    if(ret < 0){
...
static int mpeg_decode_end(AVCodecContext *avctx)
{
    ...
    if (s->mpeg_enc_ctx_allocated)
 	...
 	{
 		MpegEncContext *s2 = &s->mpeg_enc_ctx;
 		/*x00141957 20100507 增加对parse_context.buffer内存的释放*/
...
/* 新增函数：复位mpeg2解码器 xiongfei20100302*/
static av_cold int mpeg12_reset(Mpeg1Context *h)
{
	...
	/* 清空Mpeg1Context结构体变量 xionigfei20100302*/
	memset(h, 0, sizeof(Mpeg1Context));

	/* 初始化Mpeg1Context结构体变量 xiongfei20100302*/
...
/*增加decode_reset函数，用于control中的reset xiongfei20100302*/
...
//	int i;
	...
	/*x00141957 20100606 测试*/
	/*问题单号：AZ1D02136
	修改人：熊飞 +00141957
	时间：2010/6/7
	问题描述： 丢包误码循环解码的情况下，码流跑死
	问题修改： reset时有内存泄露存在*/
	MPV_common_end(s);

	/* 复位及初始化AVCodecContext结构体变量 xiongfei20100302*/
	...
	/* 复位及初始化Mpeg1Context结构体变量 xiongfei20100302*/
	iRet = mpeg12_reset(h);
	if(0 != iRet)
	{
...
/*根据刘小伟改写结构体 改写这里 xiongfei20100212*/
