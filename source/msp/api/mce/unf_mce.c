...
HI_S32 HI_UNF_MCE_Init(HI_UNF_MCE_INIT_PARAM_S *pstInitParam)
{
    ...
    if(HI_FAILURE == stat(g_MceDevName, &st))
    {
        HI_ERR_MCE("ERR: mce device is not exit!");
        HI_MCE_UNLOCK();
        return HI_ERR_MCE_DEV_NOT_EXIST;
    }

    if(!S_ISCHR (st.st_mode))
    {
        HI_ERR_MCE("ERR: mce is not device.\n");
        HI_MCE_UNLOCK();
        return HI_ERR_MCE_NOT_DEVICE;
    } 
