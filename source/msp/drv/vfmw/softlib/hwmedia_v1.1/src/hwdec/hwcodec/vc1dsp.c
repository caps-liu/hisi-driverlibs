...
//	int d1, d2;
    ...
    for(i = 0; i < 8; i++) 
	{
        ...
        c = src[0];
        d = src[stride];
		/* 2010/07/29 10:30:00 liuxw+00139685 */
		/* 由于计算公式不正确，故进行修改 */
/*      d1 = (a - d + 3 + rnd) >> 3;
        d2 = (a - d + b - c + 4 - rnd) >> 3;
		...
		src[-2*stride] = (7*a              +   d + 3 + rnd) >> 3;
		src[-stride]   = (-a  + 7*b +   c  +   d + 4 - rnd) >> 3;
		src[0]         = ( a  +   b + 7*c  -   d + 3 + rnd) >> 3;
		src[stride]    = ( a               + 7*d + 4 - rnd) >> 3;
