...
/* 定义一个宏，用于决定h->implicit_weight的第三维的索引 */
...
typedef struct H264Context{
	...
	/*hanqingrui add only useing test*/
	uint8_t (*non_zero_count_8x8)[16];
	/*hanqingrui add end*/
...
#define PART_NOT_AVAILABLE -2
    ...
    int delta_poc[2];
    int frame_num;
    unsigned int prev_poc_msb;             ///< poc_msb of the last reference pic for POC type 0
    unsigned int prev_poc_lsb;             ///< poc_lsb of the last reference pic for POC type 0
	...
	/* 2010/04/27 19:00:00 liuxw+00139685 */
	/* 将二维的数组转换为三维的数组：在MBAFF的时候，计算隐式加权预测的时候会用到(顶场一个，底场一个) */
//  int implicit_weight[48][48];
	...
	/* 将dist_scale_factor数组由16改为32 */
//  int dist_scale_factor[16];
	int dist_scale_factor[32];
    int dist_scale_factor_field[2][32];
	/* 2010/04/20 9:30:00 liuxw+00139685 [AZ1D01992] */
	/* 将map_col_to_list0增加一个维度，表示场(当前图像是帧，ref1是场的情形) */
//  int map_col_to_list0[2][16+32];
	/* 2010/08/24 11:00:00 liuxw+00139685 */
	/* 由于ref1可能会存在多个slice，而且每个slice的参考队列有可能不同，所以当不同的宏块处于不同的slice时，应该选择正确的slice时的参考队列 */
//	int map_col_to_list0[2][2][16+32];
//  int map_col_to_list0_field[2][2][16+32];
	/* 2010/08/26 10:00:00 liuxw+00139685 */
	/* 将最高维的二维改回一维，因为两维并不会同时使用 */
//	int map_col_to_list0[2][MAX_SLICES][2][16+32];
	...
	/* 增加一个显示的buf管理、增加一个delay_pic的个数，增加一个display_pic的个数，增加一个delay_pic中参考帧的个数，增加一个display_pic中参考帧的个数 */
	Picture *display_pic[MAX_DELAYED_PIC_COUNT+2]; 
	int delay_pic_num;
	int delay_pic_ref_num;
	int display_pic_num;
	int display_pic_ref_num;
...
/* 将静态函数改为非静态函数，并进行声明，以便被其它文件中的函数使用 */
