 ...
 ** CNcomment: 重新计算缩放比例，使得跟硬件保持一致，否则会导致解码错误  CNend\n
 **/
#ifdef CONFIG_JPEG_HARDDEC2ARGB
...
while( ((cinfo->output_width > 4095) || ( cinfo->output_height > 4095 )) && (HI_FALSE == bDecARGB) && (HI_FALSE == pJpegHandle->bOutYCbCrSP))
#else
while( ((cinfo->output_width > 4095) || ( cinfo->output_height > 4095 )) && (HI_FALSE == pJpegHandle->bOutYCbCrSP))
#endif
{ 
	u32Scale++;
	if(u32Scale > 3)
   ...
   * This saves time if the upsampler get to use 1:1 scaling.
  ...
  if (! cinfo->raw_data_out) {
    if (master->using_merged_upsample) {
      ...
      ERREXIT(cinfo, JERR_NOT_COMPILED);
		...
		if(   (NULL != pJpegHandle->stOutDesc.stOutSurface.pOutVir[0])
			||(JCS_YUV400_SP	 == cinfo->out_color_space)
		...
		{
			 jinit_color_deconverter_userbuf(cinfo);
		}
		else
		{
