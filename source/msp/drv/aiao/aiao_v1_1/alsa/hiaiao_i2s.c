...
static __devinit int hisi_i2s_dev_probe(struct platform_device *pdev)
{
	return snd_soc_register_dai(&pdev->dev, &hisi_i2s_dai);
}

static __devexit int hisi_i2s_dev_remove(struct platform_device *pdev)
{
...
static struct platform_driver hisi_i2s_driver = {
	...
	.remove = __devexit_p(hisi_i2s_dev_remove),
};
