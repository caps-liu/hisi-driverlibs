...
jpeg_CreateDecompress (j_decompress_ptr cinfo, int version, size_t structsize)
{
  ...
  ** CNcomment: 可以不做此判断，每个版本都适用 CNend\n
  **/
  ...
  ** CNcomment: 初始化硬件解码需要的结构体，必须在初始化内存函数之后初始化 CNend\n
  **/
    ...
    if (cinfo->saw_Adobe_marker) 
	{
      switch (cinfo->Adobe_transform) 
	  {
	      case 0:
			...
			pJpegHandle->enImageFmt = JPEG_FMT_CMYK;
			break;
	      case 2:
			cinfo->jpeg_color_space = JCS_YCCK;
			pJpegHandle->enImageFmt = JPEG_FMT_YCCK;
			break;
	      default:
			WARNMS1(cinfo, JWRN_ADOBE_XFORM, cinfo->Adobe_transform);
...
jpeg_finish_decompress (j_decompress_ptr cinfo)
{
  ...
  #ifdef CONFIG_JPEG_GETDECTIME
  s32Ret = HI_GFX_GetTimeStamp(&u32DecTime,NULL);
  if(HI_SUCCESS != s32Ret)
  {
     return FALSE;
  }
  pJpegHandle->u32DecTime = u32DecTime - pJpegHandle->u32DecTime;
  #endif
