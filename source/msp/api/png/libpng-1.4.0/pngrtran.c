...
png_set_premulti_alpha(png_structp png_ptr)
{
   png_debug(1, "in png_set_premulti_alpha");
      ...
      if (png_ptr == NULL)
      return;
   png_ptr->transformations |= PNG_PREMULTI_ALPHA;
}
#endif
...
png_set_swdec(png_structp png_ptr)
{
    ...
    pstHwStruct->u8UserDecMode = HIPNG_SWDEC_MASK;
...
defined(PNG_READ_USER_TRANSFORM_SUPPORTED)
   ...
   info_ptr->rowbytes = PNG_ROWBYTES(info_ptr->pixel_depth, info_ptr->width);
...

    {
        ...
        if (pstHwStruct)
        {
            pstHwStruct->u32Stride = info_ptr->rowbytes;
        }
    }
    info_ptr->rowbytes = ((info_ptr->rowbytes + 0xf) & 0xfffffff0);
#endif
 ...
 */
   ...
   if (png_ptr->transformations & PNG_USER_TRANSFORM)
    ...
    if (png_ptr->transformations & PNG_PREMULTI_ALPHA)
      png_do_premulti_alpha(&(png_ptr->row_info), png_ptr->row_buf + 1);
#endif
}
