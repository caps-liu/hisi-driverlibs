...
mali_physical_memory_allocator * mali_block_allocator_create(u32 base_address, u32 cpu_usage_adjust, u32 size, const char *name)
{
	...
	if (NULL != allocator)
	{
		...
		if (NULL != info)
		{
			info->mutex = _mali_osk_mutex_init( _MALI_OSK_LOCKFLAG_ORDERED, _MALI_OSK_LOCK_ORDER_MEM_INFO);
            if (NULL != info->mutex)
            {
				...
				_mali_osk_mutex_term(info->mutex);
            }
			_mali_osk_free(info);
		}
...
static mali_physical_memory_allocation_result block_allocator_allocate(void* ctx, mali_allocation_engine * engine, mali_memory_allocation * descriptor, u32* offset, mali_physical_memory_allocation * alloc_info)
{
	...
	if (_MALI_OSK_ERR_OK != _mali_osk_mutex_wait_interruptible(info->mutex)) return MALI_MEM_ALLOC_INTERNAL_FAILURE;
	...
	if ( NULL == ret_allocation )
	{
		...
		_mali_osk_mutex_signal(info->mutex);
		return result;
	}
...
static void block_allocator_release(void * ctx, void * handle)
{
	...
	if (_MALI_OSK_ERR_OK != _mali_osk_mutex_wait_interruptible(info->mutex))
	{
