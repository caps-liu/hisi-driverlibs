...
#define PTS_REC_MAX_NUM         (HI_VDEC_MAX_INSTANCE_NEW)
...
#define HI_FATAL_PTSREC         HI_FATAL_VDEC
#define HI_ERR_PTSREC           HI_ERR_VDEC
#define HI_WARN_PTSREC          HI_WARN_VDEC
#define HI_INFO_PTSREC          HI_INFO_VDEC
...
typedef struct tagPTSREC_CHAN_INFO_S
{
    ...
    HI_UNF_AVPLAY_FRMRATE_TYPE_E enFrmRateType; /* Frame rate type */
...
static PTSREC_CHAN_INFO_S s_stPtsRecChan[HI_VDEC_MAX_INSTANCE_NEW];
...
HI_S32 PTSREC_Init(HI_VOID)
{
    ...
    for (i = 0; i < PTS_REC_MAX_NUM; i++)
    {
        ...
        s_stPtsRecChan[i].enFrmRateType = HI_UNF_AVPLAY_FRMRATE_TYPE_PTS;
    }
...
HI_S32 PTSREC_DeInit(HI_VOID)
{
    ...
    for (i = 0; i < PTS_REC_MAX_NUM; i++)
    {
        ...
        {
            PTSREC_Free(i);
            HI_WARN_PTSREC("Free %d in deinit!\n", i);
            return HI_FAILURE;
        }
    }
...
HI_S32 PTSREC_Alloc(HI_HANDLE hHandle)
{
    ...
    {
        HI_ERR_PTSREC("Bad handle:%d\n", hHandle);
        return HI_FAILURE;
    }

    if (s_stPtsRecChan[hHandle].enStatus > PTSREC_CHAN_INITED)
    {
        HI_ERR_PTSREC("Already alloc:%d\n", hHandle);
        return HI_FAILURE;
    }
...
HI_S32 PTSREC_SetFrmRate(HI_HANDLE hHandle, HI_UNF_AVPLAY_FRMRATE_PARAM_S *pstFrmRate)
{
    ...
    if ((HI_UNF_AVPLAY_FRMRATE_TYPE_USER == s_stPtsRecChan[hHandle].enFrmRateType)
        || (HI_UNF_AVPLAY_FRMRATE_TYPE_USER_PTS == s_stPtsRecChan[hHandle].enFrmRateType))
    {
        // 1
 ...
 * For HI_UNF_AVPLAY_FRMRATE_TYPE_STREAM:   Only Recover PTS, use frame rate from stream;
 * For HI_UNF_AVPLAY_FRMRATE_TYPE_USER:     Only Recover PTS, use user set frame rate;
 * For HI_UNF_AVPLAY_FRMRATE_TYPE_USER_PTS: Recover PTS and detect frame rate;
 */
HI_U32 PTSREC_CalcStamp(HI_HANDLE hHandle, HI_UNF_VCODEC_TYPE_E enVdecType, IMAGE *pstImage)
{
        ...
        if (HI_INVALID_PTS != pstPtsInfo->u32LastSrcPts)
        {
            ...
            if (u32NumPass)
            {
                ...
                else
                {
                    ...
                    HI_WARN_PTSREC("pts %d exception\n", u32SrcPts);
            ...
            {
                HI_WARN_PTSREC("Frame index %d repeat\n", pstImage->frame_num);
                s_stPtsRecChan[hHandle].s32InterPtsDelta = 20;
            }
        }
        else
        {
            HI_INFO_PTSREC("Got first valid source pts:%d\n", u32SrcPts);
            s_stPtsRecChan[hHandle].s32InterPtsDelta = 20;
        }
    ...
    else
    {
        if (HI_INVALID_PTS != pstPtsInfo->u32LastSrcPts)
        {
            ...
            if (HI_UNF_AVPLAY_FRMRATE_TYPE_USER == pstPtsInfo->enFrmRateType)
            {
...
static HI_VOID PTSREC_FrameRateDetect(HI_HANDLE hHandle, IMAGE *pstImage)
{
    ...
    if (!((pstImage->optm_inf.Rwzb > 0) || 
          (HI_UNF_AVPLAY_FRMRATE_TYPE_STREAM == s_stPtsRecChan[hHandle].enFrmRateType) ||
          (HI_UNF_AVPLAY_FRMRATE_TYPE_USER == s_stPtsRecChan[hHandle].enFrmRateType)))
    {
