...
HI_S32 SYNC_Create(SYNC_CREATE_S *pSyncCreate, struct file *file)
{
    ...
    g_SyncGlobalState.SyncInfo[i].pSync = (SYNC_S *)MemBuf.u32StartVirAddr;
    g_SyncGlobalState.SyncInfo[i].SyncPhyAddr = MemBuf.u32StartPhyAddr;
    g_SyncGlobalState.SyncInfo[i].File = (HI_U32)file;
...
{    
    ...
    MemBuf.u32StartVirAddr = (HI_U32)g_SyncGlobalState.SyncInfo[SyncId].pSync;
...
HI_S32 SYNC_DRV_Close(struct inode *finode, struct file  *ffile)
{
    ...
    for (i=0; i<SYNC_MAX_NUM; i++)
    {
        if (g_SyncGlobalState.SyncInfo[i].File == ((HI_U32)ffile))
        {
            Ret = SYNC_Destroy(i);            
            if (Ret != HI_SUCCESS)
            {
...
HI_S32 SYNC_Suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    ...
    if (Ret)
    {
        HI_FATAL_SYNC("down g_SyncMutex failed.\n");
        return -1;
    }
...
{
    .pfnSYNC_VidProc        = (HI_VOID *)SYNC_VidProc,
    .pfnSYNC_PcrProc        = (HI_VOID *)SYNC_PcrProc,
    .pfnSYNC_VerifyHandle   = (HI_VOID *)SYNC_VerifyHandle
};
...
HI_S32 SYNC_MOD_Ioctl(unsigned int cmd, HI_VOID *arg)
{
    HI_S32           Ret;
