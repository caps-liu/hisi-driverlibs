...
#if defined(_MSC_VER)
    strcpy(szModName, bTrickMode ? "HW264EncTrick.dll" : "H264Encoder.dll");
    pModule->handle = (int)LoadLibrary(szModName);
#elif defined(__GNUC__)
    strcpy(szModName, bTrickMode ? "libHW264EncTrick.so" : "libHW264Enc.so");
...
void test_trickmode()
{
    ...
    CalcTrickmodeCaptureInterval(trickscale, ref_framerate, dst_framerate, &ref_frames, &dst_frames);
    printf("source %d, capture %d\n", ref_frames, dst_frames);
}
#endif
