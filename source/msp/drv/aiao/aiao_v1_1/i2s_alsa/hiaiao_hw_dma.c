...
static irqreturn_t IsrAIFunc(AIAO_PORT_ID_E enPortID,HI_U32 u32IntRawStatus,void * dev_id)
{
   ...
   // unsigned int readpos = AIAO_DF_PeriodBufSize;
    unsigned int readpos = had->sthwparam.period_size*had->sthwparam.frame_size;
    had->isr_total_cnt_c++;
...
/*static int hi_hw_dma_ack(struct snd_pcm_substream *substream)
{
    struct snd_soc_pcm_runtime *soc_rtd = substream->private_data;
    struct snd_pcm_runtime *runtime = substream->runtime;
    struct snd_soc_platform *platform = soc_rtd->platform;
    ...
    if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
    {
      ...
      if(had->ack_cnt <= 8)
      {
          ...
          time_coast = ((end_time.tv_sec - start_time.tv_sec)*1000000 + (end_time.tv_usec - start_time.tv_usec))/1000;//ms              
          if((substream->stream == SNDRV_PCM_STREAM_PLAYBACK))
             printk(KERN_ALERT "\n********ACK time is %ld ms************,ack count is %d,update write pos is %d \n", time_coast,had->ack_cnt,real_size);   //8   
      }
  
      #endif
       had->runtime_appl_ptr+=real_size;
       hi_ao_alsa_update_writeptr(had->dma_index ,&(real_size));     	
...
static int __devinit soc_hw_snd_platform_probe(struct platform_device *pdev)
{
...
static int __devexit soc_hw_snd_platform_remove(struct platform_device *pdev)
{
...
static struct platform_driver hiaudio_hw_dma_driver = {
    ...
    .remove = __devexit_p(soc_hw_snd_platform_remove),
};
