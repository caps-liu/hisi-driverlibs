...
static HI_VOID* keyled_base_addr = HI_NULL_PTR;
...
static HI_VOID keyled_timeout_handler(unsigned long data)
{
    ...
    if (keyled_timeout_cnt > 10)
    {
        ...
        {
            del_timer(&sleep_timer);
            sleep_timer.expires = jiffies + msecs_to_jiffies(keyled_dev.repkey_delaytime);
...
HI_S32 KEYLED_Init_CT1642(HI_VOID)
{
    ...
    keyled_base_addr = (HI_VOID*)IO_ADDRESS(KEYLED_PHY_ADDR);
    if (keyled_base_addr < 0)
    {
        HI_ERR_KEYLED(" ioremap failure \n");
        return -EFAULT;
    }
    HI_INFO_KEYLED("keyled_base_addr =0x%x\n", keyled_base_addr);
