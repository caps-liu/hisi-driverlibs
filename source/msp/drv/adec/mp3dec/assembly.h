 ...
 * assembly.h - basic op for Mp3 in different platforms, including win32, linux, ads
 **************************************************************************************/
...
#endif
#define FASK_MULSHIFT32

/*
 * target architecture: x86
 */
#if defined (WIN32) || defined (_WIN32) /* by (L40186) */
...
static __inline int MUL16T_32(int x, int y) /* Another name of MUL32_16T */
{
    return MULSHIFT32(y, x&0xffff0000L);
}
...
static __inline int MULFrac32_16T(int x, int y)	
{
    return MULSHIFT32(x, y&0xffff0000L)<<1;
}

static __inline int MULFrac32_16B(int x, int y)	
{
...
{
	int ret;
    __asm__ volatile ("smulwt %[Result], %[y], %[x]" : [Result] "=r" (ret) : [y] "r" (y), [x] "r" (x));
    return ret;
}
