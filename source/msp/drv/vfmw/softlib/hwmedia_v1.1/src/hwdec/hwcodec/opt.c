...
//      av_log(NULL, AV_LOG_ERROR, "Value %lf for parameter '%s' out of range\n", num, name);
		av_log(NULL, AV_LOG_WARNING, "Value %lf for parameter '%s' out of range\n", num, name);
    ...
    case FF_OPT_TYPE_INT:   *(int       *)dst= (int)(llrint(num/den)*intnum); break;
    case FF_OPT_TYPE_INT64: *(int64_t   *)dst= (int64_t)llrint(num/den)*intnum; break;
    case FF_OPT_TYPE_FLOAT: *(float     *)dst= (float)num*intnum/den;         break;
    case FF_OPT_TYPE_DOUBLE:*(double    *)dst= (double)num*intnum/den;         break;
...
//                      av_log(NULL, AV_LOG_ERROR, "Unable to parse option value \"%s\": %s\n", val, error);
						av_log(NULL, AV_LOG_WARNING, "Unable to parse option value \"%s\": %s\n", val, error);
                    return AVERROR(EINVAL);
                }
            }
            if(o->type == FF_OPT_TYPE_FLAGS){
                if     (cmd=='+') d= (int)(av_get_int(obj, name, NULL) | (int64_t)d);
                else if(cmd=='-') d= (int)(av_get_int(obj, name, NULL) &~(int64_t)d);
            }else{
    ...
    while ((opt = av_next_option(s, opt)) != NULL) {
        ...
        switch(opt->type) {
            ...
            case FF_OPT_TYPE_INT64:
                ...
                av_set_int(s, opt->name, (int64_t)opt->default_val);
            break;
            case FF_OPT_TYPE_FLOAT: {
                double val;
                val = opt->default_val;
            ...
            case FF_OPT_TYPE_RATIONAL: {
                AVRational val;
				val.den = 1;//av_d2q(opt->default_val, INT_MAX);
				val.num = 0;
