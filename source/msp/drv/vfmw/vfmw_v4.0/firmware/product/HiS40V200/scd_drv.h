...
#else
#define SCD_MSG_BUFFER  128*1024
#endif
...
#define SM_MIN_SEG_BUF_SIZE   (128*1024)
...
typedef struct  
{
    ...
    SINT32   DownMsgPhyAddr;//就是下行消息池的地址
...
/************************************************************************
  原型  SINT32 GetRawNumOffsert( RAW_ARRAY_S *pRawStreamArray, SINT32 Len, SINT32 *pNum, SINT32 *pOffset, UINT64 *pPts, UINT64 *pUsertag, UINT64 *pRawPts)
  功能  从长度Len计算从第一码流包开始，共有需要有多个的码流包的大小等于Len，如果有半包则长度余数为Offset。 
  参数  pRawStreamArray 指向原始码流集合
        Len             长度
  返回值  成功返回FMW_OK，否则返回错误码
************************************************************************/
SINT32 GetRawNumOffsert( RAW_ARRAY_S *pRawStreamArray, SINT32 Len, SINT32 *pNum, SINT32 *pOffset, UINT64 *pPts, UINT64 *pUsertag, UINT64 *pDisptime, UINT64 *pRawPts);
...
/*  WIN32 、linux用户态、VXWORKS环境下进行测试时，SCD的驱动入口  */
