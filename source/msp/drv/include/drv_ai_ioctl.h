
#ifndef __DRV_AI_IOCTL_H__
 #define __DRV_AI_IOCTL_H__

#ifdef __cplusplus
#if __cplusplus
extern "C"{
#endif
#endif /* __cplusplus */

#include "hi_type.h"
#include "hi_unf_ai.h"

typedef struct hiAI_GetDfAttr_Param_S
{
    HI_UNF_AI_E              enAiPort;
    HI_UNF_AI_ATTR_S         stAttr;
} AI_GetDfAttr_Param_S, *AI_GetDfAttr_Param_S_PTR;

typedef struct hiAI_Create_Param_S
{
    HI_UNF_AI_E              enAiPort;
    HI_UNF_AI_ATTR_S         stAttr;
    HI_HANDLE                hAi;
//#ifdef HI_ALSA_AI_SUPPORT
    HI_BOOL                  bAlsaUse;      //if bAlsaTrack = HI_TRUE ALSA, or UNF
    //AI_ALSA_Param_S           stAlsapara;
    HI_VOID                  *pAlsaPara;
//#endif 
} AI_Create_Param_S, *AI_Create_Param_S_PTR;

typedef struct hiAI_Enable_Param_S
{
    HI_HANDLE          hAi;
    HI_BOOL            bAiEnable;
} AI_Enable_Param_S, *AI_Enable_Param_S_PTR;

typedef struct hiAI_GetFrame_Param_S
{
    HI_HANDLE          hAi;
    HI_UNF_AO_FRAMEINFO_S   stAiFrame;
} AI_Frame_Param_S, *AI_Frame_Param_S_PTR;

typedef struct hiAI_Attr_Param_S
{
    HI_HANDLE                hAi;
    HI_UNF_AI_ATTR_S         stAttr;
} AI_Attr_Param_S, *AI_Attr_Param_S_PTR;

typedef struct hiAI_Buf_Param_S
{
    HI_HANDLE                hAi;
    AI_BUF_ATTR_S            stAiBuf;
} AI_Buf_Param_S, *AI_Buf_Param_S_PTR;

/*AI Device command code*/
#define CMD_AI_GEtDEFAULTATTR _IOWR  (HI_ID_AI, 0x00, AI_GetDfAttr_Param_S)
#define CMD_AI_CREATE _IOWR  (HI_ID_AI, 0x01, AI_Enable_Param_S)
#define CMD_AI_DESTROY _IOW  (HI_ID_AI, 0x02, HI_HANDLE)
#define CMD_AI_SETENABLE _IOW  (HI_ID_AI, 0x03, HI_UNF_AI_ATTR_S)
#define CMD_AI_ACQUIREFRAME _IOWR  (HI_ID_AI, 0x04, AI_Frame_Param_S)
#define CMD_AI_RELEASEFRAME _IOW  (HI_ID_AI, 0x05, AI_Frame_Param_S)
#define CMD_AI_SETATTR _IOW  (HI_ID_AI, 0x06, AI_Attr_Param_S)
#define CMD_AI_GETATTR _IOWR  (HI_ID_AI, 0x07, AI_Attr_Param_S)
#define CMD_AI_GETBUFINFO _IOWR  (HI_ID_AI, 0x08, AI_Buf_Param_S)
#define CMD_AI_SETBUFINFO _IOW  (HI_ID_AI, 0x09, AI_Buf_Param_S)

#ifdef __cplusplus
#if __cplusplus
}
#endif
#endif /* __cplusplus */
#endif 
 
