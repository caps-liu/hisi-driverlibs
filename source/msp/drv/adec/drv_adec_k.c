...
#define ADEC_NAME "HI_ADEC"
 ...
 #define CHECK_ADEC_OPEN(hAdec) \
    ...
    {  int i;                                                 \
       ...
       { \
           if (((HI_HANDLE)(&g_stAdecSlimChan[i])) == hAdec) \
           { \
               break; \
           } \
       } \
...
static HI_S32 ADECKelInitOutBuf(ADEC_STREAM_OUTBUF_S  *pstOutBuf, HI_U32 u32OutBufNum, HI_U32 uMaxBufSizePerFrame )
{
    ...
    {
        pstOutBuf->outBuf[i].OutBuf = (HI_U32*)(((HI_U32)pu32Buf) + i * uMaxBufSizePerFrame);
                                ...
                                HI_U32 u32BsSize )
{
    ...
    if (0xffffffff == u32PtsMs)
    {
        ...
        {
            ptPTSArray[pstPTSQue->ulPTSwrite
                       - 1].u32EndPos = (ptPTSArray[pstPTSQue->ulPTSwrite
                                                    - 1].u32EndPos + u32BsSize) % pstInBuf->u32BufSize;
        }
        else
        {
            ptPTSArray[ADEC_MAX_STORED_PTS_NUM
                       - 1].u32EndPos = (ptPTSArray[ADEC_MAX_STORED_PTS_NUM
                                                    - 1].u32EndPos + u32BsSize) % pstInBuf->u32BufSize;
        }
    }
...
static HI_S32 ADECKelCloseChannel(ADEC_SLIM_CHAN_S *pstAdecChan)
{
    ...
    {
        HI_KFREE(HI_ID_ADEC, (HI_VOID *)pstAdecChan->hAdecArry);
        pstAdecChan->hAdecArry = 0;
    }
...
HI_S32 HI_DRV_ADEC_Open(HI_HANDLE *phAdec, ADEC_SLIM_ATTR_S *pstAdecAttr)
{
    ...
    pstAdecChan->hAdecArry = (HI_U32)ptrmem;
    ADECKelResetMidState(pstAdecChan);
    *phAdec = (HI_HANDLE )pstAdecChan;
    return HI_SUCCESS;
}
...
HI_S32 HI_DRV_ADEC_SetAttr(HI_HANDLE hAdec, ADEC_SLIM_ATTR_S *pstAdecAttr)
{
    ...
    pstAdecChan = (ADEC_SLIM_CHAN_S *)hAdec;
...
HI_S32 HI_DRV_ADEC_Reset(HI_HANDLE hAdec)
{
    ...
    ADECKelResetChannel((ADEC_SLIM_CHAN_S *)hAdec);
...
HI_S32 HI_DRV_ADEC_ReceiveFrame (HI_HANDLE hAdec, HI_UNF_AO_FRAMEINFO_S *pstAOFrame, ADEC_EXTFRAMEINFO_S *pstExtInfo)
{
    ADEC_SLIM_CHAN_S *pstAdecChan = HI_NULL_PTR;
    ADEC_STREAM_OUTBUF_S  *pstOutBuf = HI_NULL_PTR;
