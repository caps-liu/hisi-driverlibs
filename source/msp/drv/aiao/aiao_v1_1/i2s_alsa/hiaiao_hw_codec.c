...
void aiao_hw_codec_deinit(void)
{
	remove_i2c();
...
static int __devinit aiao_codec_probe(struct platform_device *pdev)
{
	return snd_soc_register_codec(&pdev->dev,
			&soc_codec_dev_aiao, &aiao_dai, 1);
}

static int __devexit aiao_codec_remove(struct platform_device *pdev)
{
...
static struct platform_driver aiao_codec_driver = {
	...
	.remove = __devexit_p(aiao_codec_remove),
};
...
int aiao_hw_codec_init(void)
{
    ...
    //register device
    aiao_codec_device = platform_device_alloc("hi-aiao-i2s", -1);
    if (!aiao_codec_device) {
    	//printk(KERN_ERR "hi-aiao SoC Codec platform device: Unable to register\n");
    	return -ENOMEM;
    }
    ret = platform_device_add(aiao_codec_device);
    if (ret) {
    	//printk(KERN_ERR "hi-aiao SoC Codec platform device: Unable to add\n");
