...
HI_VOID SIL9293_STABLE_GetTiming(HI_HDMIRX_TIMING_INFO_S *pstTiming)
{
    ...
    pstTiming->enOverSampleMode = HI_OVERSAMPLE_1X;
...
#if 0
    if (g_stHdmirxCtx.u32LoopCnt % 10 == 0)
    {
...
HI_VOID SIL9293_UpIPState(HI_VOID)
{
    ...
    /* first time of clock */
    if (!g_stHdmirxCtx.bHasClkEven)
    {
        g_stHdmirxCtx.bHasClkEven = HI_TRUE;
        g_stHdmirxCtx.enIpState = HDMIRX_IP_STATE_WAITCONNET;
        return;
    }

    SIL9293_HAL_GetStable(&bStable);
    ...
    if (!bStable)
    {
        if (HI_HDCP_STATE_SUCCESS != g_stHdmirxCtx.enHdcpState)
        {
            g_stHdmirxCtx.enIpState = HDMIRX_IP_STATE_NOSTABLE;
        }
        else
        {
            ...
            HDMIRX_TRACE(HI_DBG_ERR,
                         "u32HTotal:%d, u32VTotal:%d\n", u32HTotal, u32VTotal);
