...
*/
    if ( (nVals & 0x01) || (tabIdx >= HUFF_PAIRTABS) || (tabIdx < 0) || (tabType == invalidTab) )
    {
        return -1;
    }
...
#ifdef MP3DEC_DEBUG
{
    ...
    {
        huffOut = hi->huffDecBuf[ch][i];
        if (huffOut > 32767 || huffOut < (-32768))
        {
            cnt = cnt;
        }
        tmp = huffOut & 0xffff; /* extract the low 16bit, not consider the sign of 31bit */
        if (huffOut < 0)
        {
            tmp *= -1;
        }
        fwrite(&tmp, sizeof(short), 1, fHufOut);
        cnt++;
        if (cnt == 2309)
        {
            cnt = cnt;
