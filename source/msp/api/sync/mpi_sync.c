...
HI_S32 HI_MPI_SYNC_Init(HI_VOID)
{
    ...
    if (HI_FAILURE == stat(g_SyncDevName, &st))
    {
        HI_FATAL_SYNC("SYNC is not exist.\n");
        HI_SYNC_UNLOCK();
        return HI_ERR_SYNC_DEV_NOT_EXIST;
    }

    if (!S_ISCHR (st.st_mode))
    {
        HI_FATAL_SYNC("SYNC is not device.\n");
        HI_SYNC_UNLOCK();
        return HI_ERR_SYNC_NOT_DEV_FILE;
    }
...
HI_S32 HI_MPI_SYNC_Create(HI_UNF_SYNC_ATTR_S *pstSyncAttr, HI_HANDLE *phSync)
{
    ...
    SyncUsrAddr.SyncUsrAddr = (HI_U32)pSync;
...
HI_S32 HI_MPI_SYNC_SetDDPTestMode(HI_HANDLE hSync, HI_BOOL bEnable)
{
    ...
    (HI_VOID)SYNC_Mutex_Lock((pthread_mutex_t *)pSync->pSyncMutex);
    
    pSync->AudDDPMode = bEnable;

    (HI_VOID)SYNC_Mutex_UnLock((pthread_mutex_t *)pSync->pSyncMutex);
    return HI_SUCCESS;
}
...
HI_S32 HI_MPI_SYNC_SetExtInfo(HI_HANDLE hSync, SYNC_EXT_INFO_E enExtInfo, HI_VOID *pData)
{
    ...
    case SYNC_EXT_INFO_FIRST_PTS:
        pSync->VidFirstDecPts = (HI_U32)pData;
        break;
    case SYNC_EXT_INFO_SECOND_PTS:
        pSync->VidSecondDecPts = (HI_U32)pData;
        break;
    default:
        break;
    }
