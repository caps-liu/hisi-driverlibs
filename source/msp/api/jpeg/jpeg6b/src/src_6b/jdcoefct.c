...
decompress_onepass (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  ...
  #if defined(CONFIG_JPEG_ADD_GOOGLEFUNCTION) && defined(ANDROID_TILE_BASED_DECODE)
	if (cinfo->tile_decode) 
	{
	  last_MCU_col =
		  (cinfo->coef->MCU_column_right_boundary - cinfo->coef->MCU_column_left_boundary) - 1;
	}
  #endif  /** CONFIG_JPEG_ADD_GOOGLEFUNCTION ANDROID_TILE_BASED_DECODE **/
       ...
       yoffset++) 
  {
	    ...
	    for (MCU_col_num = coef->MCU_ctr; MCU_col_num <= last_MCU_col;MCU_col_num++) 
		{
		      /* Try to fetch an MCU.  Entropy decoder expects buffer to be zeroed. */
				  ...
				  if ((int)MCU_col_num < coef->pub.MCU_columns_to_skip) 
				  {
...
consume_data (j_decompress_ptr cinfo)
{
  ...
  for (yoffset = coef->MCU_vert_offset; yoffset < coef->MCU_rows_per_iMCU_row;
	...
	#ifdef CONFIG_JPEG_ADD_GOOGLEFUNCTION
        /** zero all blocks **/
	    for (MCU_col_num = coef->MCU_ctr; MCU_col_num < MCUs_per_row;MCU_col_num++)
	#else
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    for (block_row = 0; block_row < block_rows; block_row++) {
	  buffer_ptr = buffer[block_row];
	      ...
	      for (block_num = start_block; (int)block_num < width_in_blocks; block_num++)
	  #else
	      output_col = 0;
