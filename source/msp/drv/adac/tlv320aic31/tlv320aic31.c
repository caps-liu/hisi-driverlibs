...
#include "hi_reg_common.h"
 ...
 #undef  USE_GPIO_I2C
 static int i2cchn = 0;
#elif defined (CHIP_TYPE_hi3712)  //x6 fpga
 #define BOARD_TYPE_fpga
 #undef  USE_GPIO_I2C
 static int i2cchn = 1;
...
{
#if 0 /* v100 use gpio_i2c_write(OK), v101 use gpio_i2c_write_ext(OK) */
...
void tlv320_SetBitWidth(HI_U32 SioMode, HI_U32 BitWidth)
{
    if (SioMode == IIS)
    {
        if (BitWidth == 16)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x07);
        }
        else if (BitWidth == 20)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x17);
        }
        else if (BitWidth == 24)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x27);
        }
        else if (BitWidth == 32)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x37);
        }
    }
    else if (SioMode == DSP)
    {
        if (BitWidth == 16)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x47);
        }
        else if (BitWidth == 20)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x57);
        }
        else if (BitWidth == 24)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x67);
        }
        else if (BitWidth == 32)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x77);
        }
    }
    else if (SioMode == RIGHT_JUSTIFIED)
    {
        if (BitWidth == 16)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x87);
        }
        else if (BitWidth == 20)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0x97);
        }
        else if (BitWidth == 24)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0xa7);
        }
        else if (BitWidth == 32)
        {
            tlv320aic31_write(I2C_AIC31, 9, 0xb7);
        }
    }
}
...
void tlv320_SetSlotOffset(HI_U32 u32SlotOffset)
{
    tlv320aic31_write(I2C_AIC31, 10, u32SlotOffset & 0xff);
}

tlv320_SetBclkMode(HI_U32 Bclk_Mode)
{
...
TLV_CODEC_SetBclkMode(HI_S32 CodecId, HI_S32 BclkMode)
{
    switch(BclkMode)
    {
