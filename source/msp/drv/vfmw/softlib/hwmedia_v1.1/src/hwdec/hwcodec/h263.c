...
/*由于H.263支持16cif格式1408x1152,高度超过full HD的1080,故在解码时重新按H.263的协议进行检测*/
...
{
	int format, coded_frame_rate, coded_frame_rate_base, i, temp_ref = 1;
	...
	if(s->h263_plus){
		for(i=0; i<2; i++){
			int div, error;
			div= 64;//(s->avctx->time_base.num*1800000LL + 500LL*s->avctx->time_base.den) / ((1000LL+i)*s->avctx->time_base.den);
...
void ff_mpeg4_init_direct_mv(MpegEncContext *s)
{
	...
	for(i=0; i<tab_size; i++){
		/*guoshan+00101841 20100730 问题单：[AZ1D02236]*/
		/*s->pp_time误码情况下出现0值时，做除数，导致floating-point exception问题*/
		/*s->pp_time = 0时，mv直接置0*/
// 		s->direct_scale_mv[0][i] = (i-tab_bias)*s->pb_time/s->pp_time;
// 		s->direct_scale_mv[1][i] = (i-tab_bias)*(s->pb_time-s->pp_time)/s->pp_time;
		if(s->pp_time)
		{
			s->direct_scale_mv[0][i] = 0 /*(i-tab_bias)*s->pb_time/s->pp_time*/;
			s->direct_scale_mv[1][i] = 0/*(i-tab_bias)*(s->pb_time-s->pp_time)/s->pp_time*/;
		}
		else
		{
			av_log(s->avctx, AV_LOG_WARNING, "s->pp_time = 0, can't be divided.\n");
			s->direct_scale_mv[0][i] = 0;
			s->direct_scale_mv[1][i] = 0;
		}
...
int ff_mpeg4_set_direct_mv(MpegEncContext *s, int mx, int my)
{
	...
	if(IS_8X8(colocated_mb_type))
	{
		...
		for(i=0; i<2; i++){
			...
			/*guoshan+00101841 20100730 问题单：[AZ1D02236]*/
			/*time_pp误码情况下出现0值，做除数，导致floating-point exception问题*/
			/*time_pp = 0时，mv直接置0*/
			...
			{
				av_log(s->avctx, AV_LOG_WARNING, "time_pp = 0, can't be divided.\n");
				s->mv[0][i][0] = 0;
				s->mv[0][i][1] = 0;
				s->mv[1][i][0] = 0;
				s->mv[1][i][1] = 0;
						  ...
						  int *px, int *py)
{
	...
	if (s->first_slice_line && block<3) {
		...
		if(block==0){ //most common case
			...
			}else{ //guoshan+00101841 20100511:第一行的mb的pred_mv基本是其左边块的mv
				*px = A[0];
				*py = A[1];
			}
		}else if(block==1){
		...
		B = mot_val[ - wrap];  //guoshan+00101841 20100511:上边块mv
		C = mot_val[off[block] - wrap];
		*px = mid_pred(A[0], B[0], C[0]);
		*py = mid_pred(A[1], B[1], C[1]);
	}
...
void h263_encode_init(MpegEncContext *s)
{
	...
	case CODEC_ID_MPEG4:
		...
		if(s->flags & CODEC_FLAG_GLOBAL_HEADER){
			...
			s->avctx->extradata= av_malloc_hw(1024);
	...
	else
	{
		...
		if(s->error_recognition>=3)
		{
			if(level<0)
			{
				...
				/*guoshan+00101841 20100423添加错误码*/
...
static int h263_decode_gob_header(MpegEncContext *s)
{
	...
	for(;left>13; left--)    //guoshan+00101841: 13bits = GBSC最后1位bit + GN(5bits) + GFID(2bits) + GQUANT(5bits)
	{
		if(get_bits1(&s->gb)) break; /* Seek the '1' bit */
	}
	if(left<=13)
	{
		/*guoshan+00101841 20100423添加日志信息*/
...
/*mpeg4 partition a is for I-VOP*/
...
*/
/*guoshan+00101841 20100512*/
/*mpeg4 partition b is for P-VOP*/
	...
	if (s->pict_type == FF_P_TYPE) 
	{
		do
		{
			if (get_bits1(&s->gb)) 
			{
				...
				/* 如果obmc功能打开，那么当前是skip还需要计算右边宏块的mv */
				if(s->obmc)
				{
					s->current_picture.motion_val[0][s->block_index[0]][0] = s->current_picture.motion_val[0][s->block_index[0]][1] =
					s->current_picture.motion_val[0][s->block_index[0]+1][0] = s->current_picture.motion_val[0][s->block_index[0]+1][1] =
					s->current_picture.motion_val[0][s->block_index[0]+s->b8_stride][0] = s->current_picture.motion_val[0][s->block_index[0]+s->b8_stride][1] =
					s->current_picture.motion_val[0][s->block_index[0]+s->b8_stride+1][0] = s->current_picture.motion_val[0][s->block_index[0]+s->b8_stride+1][1] = 0;
					goto OBMC;
				}
				goto end;
			}
		...
		if ((cbpc & 16) == 0) 
		{
			...
			if (mx >= 0xffff) //guoshan+00101841 20100524 H.263mv取值[0, 0xFFFF],参考6486 M4H3DEC_HW_decodeMV()
			{
				//guoshan+00101841 20100524增加mv错误打印信息
			...
			if(/*s->mb_x == 0  && s->mb_y == 0*/s->obmc)
			{
				...
				mot_val[1] = mot_val[3] = mot_val[1+s->b8_stride*2] = mot_val[3+s->b8_stride*2] = my;
			}
		} 
		else 
		{
		...
		for (i = 0; i < 6; i++) 
		{
			if (h263_decode_block(s, block[i], i, cbp&32) < 0)
				return -1;
			cbp+=cbp; //guoshan+00101841 相当于cbp <<=1;
		}
OBMC:
		if(s->obmc)
		{
	...
	if (s->pict_type == FF_P_TYPE || s->pict_type==FF_S_TYPE) 
	{
		do{
			...
			if (cbpc < 0)
			{
				...
				//av_log(s->avctx, AV_LOG_ERROR, "cbpc damaged at %d %d\n", s->mb_x, s->mb_y);
				av_log(s->avctx, AV_LOG_WARNING, "cbpc damaged at %d %d\n", s->mb_x, s->mb_y);
				/*guoshan+00101841 20100424添加错误码*/
		...
		if(IS_3IV1){  
			if(code==1)
				...
				if(get_bits1(&s->gb))
					...
					level =  -(1<<(code-1))-get_bits(&s->gb, code-1);
			}
		}else{
...
int h263_decode_picture_header(MpegEncContext *s)
{
	...
	/* 由于结构调整，当前current_picture_ptr还未获取 */
...
//		return -1;      /* h263 id */
	...
	*/
	...
	if (format != 7 && format != 6) 
	{
		...
		if (!width) /*guoshan+00101841 20100423根据查表结果，width==0时意味同时height==0，不用重复判断，同时format = 0的错误在此处返回*/
		{
...
//			return -1;
		...
		/*guoshan+00101841 20100612解码器不用考虑兼容ZYGO*/
// 		case 7: 
// 			{
// 				s->pict_type= FF_I_TYPE;
// 				break; //ZYGO
// 			}
		...
		if (ufep) 
		{
			...
			if ((width == 0) || (height == 0))
			{
				...
				IMEDIA_SET_ERR_SEQ( s->avctx->iErrorCode, IMEDIA_ERR_SEQ_SIZE);
				s->avctx->iTotalError++;
				/*guoshan+00101841 20100612不用return,后面会对s->width进行限制的*/
//				return -1;
			}
			s->width = width;
			s->height = height;
			...
			if(s->custom_pcf)
			{
				...
				{
					/*guoshan+00101841 20100612不用return,修正为一个值*/
					/*guoshan+00101841 20100613, 日志打印的指针出错*/
...
//				gcd= (int)av_gcd(s->avctx->time_base.den, s->avctx->time_base.num); //s->avctx->time_base.den是非0常数
	...
	if((0 == s->avctx->usActualWidth)||(0 == s->avctx->usActualHeight))
	{
		...
		int height_high = (!s->avctx->usSourceHeight) ? H263_MAX_HEIGHT : s->avctx->usSourceHeight;

		if(s->width > width_high || s->width < IVIDEO_MIN_WIDTH
			|| s->height > height_high || s->height < IVIDEO_MIN_HEIGHT)
		{
			av_log(s->avctx, AV_LOG_WARNING, "width[%d] or height[%d] incorrect!\n",s->width, s->height);
			IMEDIA_SET_ERR_SEQ(s->avctx->iErrorCode, IMEDIA_ERR_SEQ_SIZE);
			s->avctx->iTotalError++;
			return AVCODEC_RET_DECODE_FAILURE;
		}
		...
		s->avctx->iChromaFormat = s->chroma_format = 1; //1:CHROMA_420 
	}
	else
	{
		if(s->width != s->avctx->usActualWidth || s->avctx->usActualHeight != s->height)
		{
			av_log(s->avctx, AV_LOG_WARNING, "modify width[%d] to usActualWidth[%d] or height[%d] to usActualHeight[%d]!\n",
				s->width, s->avctx->usActualWidth, s->height, s->avctx->usActualHeight);
			IMEDIA_SET_ERR_SEQ(s->avctx->iErrorCode, IMEDIA_ERR_SEQ_SIZE);
			s->avctx->iTotalError++;
...
/*取消函数的静态申明，供probe_header.c中的函数直接调用*/
...
int decode_vol_header(MpegEncContext *s, GetBitContext *gb)
{
	...
	{ /* is_ol_id */
		...
		/*guoshan+00101841 20100428根据MPEG-4协议增加对video_object_layer_verid 的检测*/
...
//			return -1; 
	...
	if(s->aspect_ratio_info == FF_ASPECT_EXTENDED)
	{
		...
		/*guoshan+00101841 20100428根据MPEG-4协议增加对par_width和par_height的检测*/
		if(!s->avctx->sample_aspect_ratio.num || !s->avctx->sample_aspect_ratio.den)
		{
			...
			IMEDIA_SET_ERR_SEQ( s->avctx->iErrorCode, IMEDIA_ERR_SEQ_SIZE);
			s->avctx->iTotalError++;
			/*guoshan+00101841 20100612 par_width,par_height对解码无影响，可直接修正，不用返回*/
...
//			return -1; 
	...
	{ /* vol control parameter */
		...
		// 			return -1; //guoshan + 00101841 非4：2：0格式不支持，直接退出
		//         }
	...
	s->avctx->time_base.den = get_bits(gb, 16);  /*guoshan+00101841 20100428 vop_time_increment_resolution, called ticks*/
...
//		return -1;
	...
	if (s->shape != BIN_ONLY_SHAPE) 
	{
		if (s->shape == RECT_SHAPE) 
		{
			...
			/*guoshan+00101841 20100423添加防错处理, 日志，错误码*/
			if(width == 0 || height == 0)
			{
				av_log(s->avctx, AV_LOG_WARNING, "width[%d] or height[%d] == 0\n", width, height);
				IMEDIA_SET_ERR_SEQ( s->avctx->iErrorCode, IMEDIA_ERR_SEQ_SIZE);
			...
			/*guoshan+00101841 20100423下面的判断条件可以取消了*/
			...
			/*guoshan + 00101841 20100416*/
			/*把解析后的宽高信息进行防错检测，并保存出来*/
			//s->avctx->iChromaFormat = s->chroma_format;

			if((0 == s->avctx->usActualWidth)||(0 == s->avctx->usActualHeight))
			{
				int width_high = (!s->avctx->usSourceWidth) ? IVIDEO_MAX_WIDTH : s->avctx->usSourceWidth; 
				int height_high = (!s->avctx->usSourceHeight) ? IVIDEO_MAX_HEIGHT : s->avctx->usSourceHeight;
				...
				/*当码流中无vol_control_parameter字段时,s->chroma_format始终未被赋值,导致后续解析错误,第一次解析时要赋上正确值*/
		...
		s->progressive_sequence=
			s->progressive_frame= get_bits1(gb)^1;  /*guoshan+00101841 20100428 interlaced flag*/
		s->interlaced_dct=0;
		...
		if(s->vol_sprite_usage==STATIC_SPRITE) 
		{
			...
			/*guoshan+00101841 20100612 不支持Static Sprites功能，关掉*/
		...
		if(!get_bits1(gb))
		{
			...
			if(estimation_method<2)
			{
				if(!get_bits1(gb))
				{
					...
					s->cplx_estimation_trash_i += 8*get_bits1(gb); //upsampling
				}
				if(!get_bits1(gb))
				{
...
static int decode_vop_header(MpegEncContext *s, GetBitContext *gb)
{
	...
	if(s->time_increment_bits==0 || !(show_bits(gb, s->time_increment_bits+1)&1))
	{
		...
		av_log(s->avctx, AV_LOG_WARNING, "my guess is %d bits ;)\n",s->time_increment_bits);
	}
	...
	if(s->pict_type!=FF_B_TYPE)
	{
		...
		s->pp_time= (uint16_t)(s->time - s->last_non_b_time);
		s->last_non_b_time= s->time;
	}
	else
	{
		s->time= (s->last_time_base + time_incr)*s->avctx->time_base.den + time_increment;
		s->pb_time= (uint16_t)(s->pp_time - (s->last_non_b_time - s->time));
	...
	/*	s->pp_field_time= (uint16_t)((  ROUNDED_DIV(s->last_non_b_time, s->t_frame)
			- ROUNDED_DIV(s->last_non_b_time - s->pp_time, s->t_frame))*2);
		s->pb_field_time= (uint16_t)((  ROUNDED_DIV(s->time, s->t_frame)
			- ROUNDED_DIV(s->last_non_b_time - s->pp_time, s->t_frame))*2); */
		if(!s->progressive_sequence)
		{
	...
	{
		//guoshan+00101841 20100517修改AV_LOG_ERROR为AV_LOG_INFO
...
// 		s->avctx->iTotalError++;
		...
		/*guoshan+00101841 20100611在无B帧状态下，low_delay=0仅会影响整个序列输出时延1帧，不影响解码结果，不设置该位比较保险*/
		/*s->low_delay=1;*/
	}
...
int ff_mpeg4_decode_picture_header(MpegEncContext * s, GetBitContext *gb)
{
	...
	for(;;) 
	{
		if(get_bits_count(gb) >= gb->size_in_bits)
		{
			...
			{
				/*guoshan+00101841 20100424添加日志信息*/
				/*guoshan+00101841 20100608日志打印多余，码流结束退出，不一定是找不到startcode*/
...
#define  VIS_OBJSEQ_START_CODE 0x1B0
		if(startcode == VIS_OBJSEQ_START_CODE)
		{
			...
			case IVIDEO_PROFILE_MPEG4_SP:
				{
					if((iLevel >= IVIDEO_LEVEL_MPEG4_0) && (iLevel < IVIDEO_LEVEL_MPEG4_4))
						break;
				}
				/*else fall through to show av_log warning*/
			case IVIDEO_PROFILE_MPEG4_MP:
				{
					if((iLevel > IVIDEO_LEVEL_MPEG4_1) && (iLevel < IVIDEO_LEVEL_MPEG4_5))
						break;
				}
				/*else fall through to show av_log warning*/
			case IVIDEO_PROFILE_MPEG4_ASP:
				{
					if(iLevel <= IVIDEO_LEVEL_MPEG4_3b)
						break;
				}
				/*else fall through to show av_log warning*/
			default: 
				{
					...
					/*guoshan+00101841 20100423添加错误码,level或profile的错误都添到这里，暂不区分*/
...
//		if(startcode >= 0x120 && startcode <= 0x12F)
		else if(startcode >= 0x120 && startcode <= 0x12F)
		{
			...
			{
				/*guoshan+00101841 20100424添加日志*/
				av_log(s->avctx, AV_LOG_WARNING, "decode_vol_header failed!\n");
                /*guoshan+00101841 20100628标志iNotFoundVOLFlag*/
				s->iNotFoundVOLFlag = 1;
				return -1;
			}
			/*guoshan+00101841 20100628找到VOL头后修正该标志位*/
	...
	/*guoshan+00101841 20100628 在无正确VOL头的情况下，不必解析VOP头*/
...
static av_cold int h263_mpeg4_reset(MpegEncContext *h)
{
	...
	/* 无需全部清0，然后再去调用初始化函数，只需要将s中部分状态信息清0即可 */
#if 0
	/* 清空MpegEncContext结构体变量*/
	memset(h, 0, sizeof(MpegEncContext));

	/* 初始化MpegEncContext结构体变量*/
	...
	avctx->coded_frame= (AVFrame*)&s->current_picture;
	s->flags  = avctx->flags;
...
static av_cold int decode_reset(AVCodecContext *avctx)
{
	...
	/* 2010/09/14 16:30:00 liuxw+00139685 */
	/* 所有的内存只分配一次，所以只在close的时候才会涉及到释放 */
	//MPV_common_end(s);
...
int H263_Query_MemSize(uint16_t width, uint16_t height, uint16_t bufnum, int pixformat)
{
	int total_size = 0;
    int y_size, c_size, yc_size, mb_array_size, mv_table_size;
	int tmp_width,tmp_height;
	int mb_height,mb_width,mb_stride,b8_stride,b4_stride;
	int linesize,uvlinesize,mb_num;
	int tmp_size = 0;
	...
	total_size += sizeof(MpegEncContext); // H263解码器通道变量所占的内存大小 
    ...
    b4_stride = mb_width*4 + 1;
    mb_array_size= mb_height * mb_stride;
    mv_table_size= (mb_height+2) * mb_stride + 1;
	...
	/* channel level */
	total_size += (mb_num+1)*sizeof(int);                // mb_index2xy buf
	total_size += MAX_PICTURE_COUNT * sizeof(Picture);   // pic array buf
	total_size += yc_size * sizeof(int16_t) * 16;        // av buf
	total_size += y_size;                                // cbp buf
	total_size += mb_array_size * sizeof(uint8_t);       // cbp_table buf
	total_size += mb_array_size * sizeof(uint8_t);       // pred_dir_table buf
	total_size += yc_size * sizeof(int16_t);		     // dc_val_base buf
	total_size += mb_array_size;                         // mbintra_table
	total_size += mb_array_size+2;                       // mbskip_table
	total_size += PREV_PICT_TYPES_BUFFER_SIZE;           // prev_pict_types

	total_size += (mb_width*16 + 2*EDGE_WIDTH) * mb_height*16 + 2*EDGE_WIDTH;       // prev_pict_types
	total_size += (mb_width*16 + 2*EDGE_WIDTH) * mb_height*16 + 2*EDGE_WIDTH;       // prev_pict_types
	total_size += (mb_width*16 + 2*EDGE_WIDTH) * mb_height*16 + 2*EDGE_WIDTH;       // prev_pict_types

	total_size += (width+64)*2*21*2;                // allocated_edge_emu_buffer
	total_size += (width+64)*4*16*2*sizeof(uint8_t);// me.scratchpad
	total_size += 64*12*2 * sizeof(DCTELEM);        // blocks


	tmp_width  =  mb_width*16;
	tmp_height =  mb_height*16;

    tmp_width  += EDGE_WIDTH*2;
    tmp_height += EDGE_WIDTH*2;

	linesize   = tmp_width;
	uvlinesize = (tmp_width+1)>>1;

	big_mb_num     = mb_stride*(mb_height+1) + 1; 
    mb_array_size2 = mb_stride*mb_height;
    b8_array_size  = b8_stride*mb_height*2;
    b4_array_size  = b4_stride*mb_height*4;
	/* pic level */
	total_size += (INTERNAL_BUFFER_SIZE+1)*sizeof(InternalBuffer); // yuv manage buf

	tmp_size  = tmp_height*linesize*3/2 + tmp_height * uvlinesize; // one frame pic buf size
	tmp_size +=  mb_array_size * sizeof(uint8_t)+2;                // mbskip_table
	tmp_size +=  mb_array_size * sizeof(uint8_t);                  // qscale_table
	tmp_size +=  (big_mb_num + mb_stride) * sizeof(uint32_t);      // mb_type_base
	tmp_size +=  2 * (b8_array_size+4) * sizeof(int16_t) * 2;      // motion_val_base[2]
	tmp_size +=  b8_array_size * sizeof(uint8_t) * 2;              // ref_index[2]
	tmp_size +=  1 * sizeof(AVPanScan);                            // pan_scan
	tmp_size +=  (big_mb_num-1+mb_stride)*sizeof(uint16_t);        // slice_table_base

	total_size += tmp_size * bufnum;

	
	total_size += 16*1024;   // 假充会分配512块内存，每块非16字节对齐，那么浪费最大的空间即为16*512
...
AVCodec h263_decoder = {
	...
	NULL_IF_CONFIG_SMALL("H.263 /H.263+"),
	NULL,
	NULL,
	NULL,
	H263_Frame_Parse,        
