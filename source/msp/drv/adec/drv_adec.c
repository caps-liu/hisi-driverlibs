...
static HI_S32 ADEC_DRV_Proc(struct seq_file *p, HI_U32  u32ChNum)
{
    ...
    PROC_PRINT(p,
                    ...
                    tmp->ThreadScheTimeOutCnt);  //verify
...
#define ADEC_DEBUG_SHOW_HELP(u32ChNum) \
    ...
    {                                                          \
        ...
        printk("commad:   echo save_es start|stop > /proc/msp/adec%02d\n", u32ChNum); \
        printk("example:  echo save_es start > /proc/msp/adec%02d\n", u32ChNum); \
        printk("\nfunction: save pcm data after audio decode \n"); \
        printk("commad:   echo save_pcm start|stop > /proc/msp/adec%02d\n", u32ChNum); \
        printk("example:  echo save_pcm start > /proc/msp/adec%02d\n\n", u32ChNum); \
    } while (0)     
...
HI_VOID  ADEC_DRV_ModExit(HI_VOID)
{
    ...
    for(i = 0; i < ADEC_INSTANCE_MAXNUM; i++)
    {
         snprintf(aszBuf, sizeof(aszBuf), "adec%02d", i);
