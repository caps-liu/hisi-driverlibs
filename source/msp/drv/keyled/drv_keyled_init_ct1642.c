...
//	kmalloc(ADEC_MAX_INPUT_BLOCK_SIZE, GFP_KERNEL);
	if(!cmdline)
	{
	    printk("KEYLED_GetDisplayPara request mem failure \n");	
		return HI_FAILURE;
	}
...
static int keyled_display_init(void)
{
	...
	printk(">>>>>> keyled_display begin >>>>>>\n");

//#ifndef HI_MCE_SUPPORT
    ret = HI_DRV_CommonInit();
	if(ret)
    {
	    printk("CommonInit init err \n");	
    }
    else
    {
	    printk("CommonInit init ok \n");	
...
//#endif
	...
	if(ret){
		printk("keyled Param Init err !");
	}
    else
    {
	    printk("keyled Param Init ok\n");
    }
#if 0
//	keyled_display_begin();
    ret = KEYLED_LED_Display_CT1642(DisplayVal);
	if(ret){
		printk("keyled display param err !");
	}
#endif

    ret = KEYLED_DisplayString(LedDisplayStr);
    if(ret){
		printk("keyled display string param err !");
	}


 	printk("<<<<<< keyled_display   end <<<<<<\n");
	return HI_SUCCESS;
}
...
static void keyled_display_exit(void)
{
	...
	COMMON_exit_0();
	printk("COMMON exit ok \n");

    media_mem_exit_0();
	printk("media_mem exit ok \n");	

	himedia_exit_0();	
	printk("himedia exit ok \n");
#endif
	return;
}
